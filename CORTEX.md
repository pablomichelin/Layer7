...
`1.8.9` (artefacto ainda nao publicado; actualizar manual e procedimentos na
...
- A ultima publicacao conhecida no canal publico continua a ser o pacote
  ...
  `pkgver` (actualmente `1.8.9` no `Makefile` do port) antes da release; nao
...
- PORTVERSION no repositorio (pre-release / proximo build): 1.8.9
...
- A referencia de **instalacao publica** continua a ser o pacote `1.8.3` ate
  nova release; o branch pode carregar `PORTVERSION=1.8.9` para o proximo
  empacotamento.
