...
`1.9.80` (GitHub Releases `pablomichelin/Layer7`, tag `v1.9.80` →
`7bd1fd012e6b8dcf00780f980d62f1a955de061a`;
...
retirada do download publico / latest-only);
herda `1.9.77`
...
**BG-128** aberto; **P0-2, P1-1, P1-2, P1-3, P1-4, P2-1, allowlist `30.11` e P1-5…P1-8 + P2-12 FEITOS** no git `2026-08-14` sem deploy; código `c2b9fdb`, governação após gates neste commit; **P2-7+P2-8+P2-10 FEITOS** no git `2026-08-14` sem deploy;
...
não aberto no HEAD; sem mudança de runtime);
**P2-2 FEITO** no git `2026-08-14` sem deploy — CSRF admin fail-closed
(`Origin` / `Sec-Fetch-Site`);
**P2-13 AVALIADO** no git `2026-08-14` — meia-noite local / DST / UTC
sem correção única segura; sem mudança de runtime;
**P2-4 FEITO** no git `2026-08-14` sem deploy — incremento atómico de
`failure_count` no lock de login;
**P2-6 Bloco A FEITO** no git `2026-08-14` sem deploy — `.dockerignore`
+ `USER node` no backend (sem compose/healthcheck);
**P2-6 Bloco B FEITO** no git `2026-08-14` sem deploy — `pg_isready` +
`depends_on` `service_healthy` (sem Docker build/up);
**P0-2 residual single-use/bind FEITO** no git `2026-08-14` sem deploy —
`jti` + `admin_totp_challenges` + consumo transaccional;
**P3-1 FEITO** no git `2026-08-14` sem deploy — sessão única atómica
com lock do admin;
**P3-2 FEITO** no git `2026-08-14` sem deploy — `GET /api/auth/session`
inclui `a.totp_enabled`;
**P3-3A FEITO** no git `2026-08-14` sem deploy — `/login` disabled e
inexistente partilham 401 genérico + bcrypt + `registerLoginFailure`;
**P3-3B** FEITO no git `2026-08-14` sem deploy —
`POST`/`PUT /api/users` exige password >=12 (bootstrap já 12; `/login` não
rejeita 10);
**P3-3C** FEITO no git `2026-08-14` sem deploy —
`verifyTotp` compara HOTP/TOTP com Buffer UTF-8 + guarda de comprimento +
`timingSafeEqual`;
...
**P3-6** FEITO no git `2026-08-14` sem deploy —
...
**não** é padrão/`latest`; sem tag/retag/deploy;
**P2-14 AVALIADO** no git `2026-08-14` — **BG-152**; opção A — **FEITO
documental**; bypass ABI `-f` = política BG-106; builder FreeBSD 16
**não** provado; sem código/`PORTVERSION`;
...
**BG-159 PUBLICADO `v1.9.68` (`da54557`, `a9a49cd`, `2026-08-15`):**
catálogo EN de Configurações completo; defaults da página de bloqueio migram
...
`pablomichelin/Layer7` publica **apenas** o pacote `latest` (`1.9.80`
agora). `v1.9.79` retirada do download publico (tag git preservada).
...
publico fica **so** a ultima release de pacote. Actual `latest` = `1.9.80`.
`v1.9.79` retirada do download (latest-only / BG-164; tag git preservada).
Nao republicar `1.9.79`.
...
`releases/latest` foi `v1.9.79` ate `1.9.80`;
`SHA256=26ef9ef1b28bee63a886bb169ead27208292548b29b47149280c1a8acfcaa482`.
`v1.9.78` retirada do download (latest-only / BG-164).
**BG-174 / GUI1 PUBLICADO `v1.9.80` (`2026-08-31`):** Status nativo
(`Form(false)` / `Form_Section` / `Form_StaticText` / painéis). Frontend-only.
`releases/latest` = `v1.9.80`;
`SHA256=f7186ee3c58d6ad948b322e45098adaf06f03b0400eab29ed1dd112c2c908782`;
tag `7bd1fd012e6b8dcf00780f980d62f1a955de061a`; 7 assets F1.2.
`v1.9.79` retirada do download (latest-only / BG-164; tag git preservada).
...
**BG-174 / ADR-0037 (`2026-08-31`):** decisão humana fixa frontend nativo ao
pfSense: manter `Services > Layer7`, organização, tabs/subtabs, tema, painéis,
tabelas, formulários, alerts e botões do host. As cinco áreas são apenas
...
**FEITOS no git** (`3b18f82`) após gates PASS; sem
código/`PORTVERSION`/appliance.
**BG-174 / GUI1 piloto Status (`2026-08-31`):** apresentação nativa de
...
sem novo GO + runbook.
**BG-106 / P2-14 (BG-152):** ABI 15 vs 16 — aceite operacional com `-f`
(GUI + `install.sh`); builder FreeBSD 16 **não** provado / **não** em
operação; **não** é suporte nativo ABI 16.
...
`1.9.13`; Identity GUI em `1.9.14`; LDAP client em `1.9.15` — **não** misturar
promoção enforce com MITM.
...
**Checkpoint `2026-08-14`:** API live **`30.13`** no `.244`
(`20260814T142739Z`; git `5754bfa`; só serviço `api`; SPA `2.0.0` intocada).
Health público/origin `ok`. Rollback preferido: overlay `30.13` /
imagem `bbc74a5…` (`sha256:bbc74a53651f835d4dd0b07f2d5f97c2a3cd25e99c8e965309aa6ea018aadb9f`).
A tag `layer7-license-api:pre-30.13-20260814T142739Z` **não** é
rollback padrão/`latest` — só incidente específico de `30.13` (P2-16).
...
**P0-2, P1-1, P1-2, P1-3, P1-4, P2-1, allowlist `30.11` e P1-5…P1-8 + P2-12 FEITOS** no git (`2026-08-14`; TOTP HMAC fail-closed;
...
`LIMIT 1` + alerta se vários owners; serving content versionado;
enforce recusa check-in ON sem chave; upgrade/keep-config preserva
json/`.lic`/CA/secrets/check-in; deinstall real limpa `/var/db` + anti-DoH;
código `c2b9fdb`; governação confirmada após gates neste commit;
...
**A1/A2/M2 FEITO** no git (`2026-08-14`; `28c97ad` + governação após gates; lifecycle fail-closed;
sem P2-13; sem deploy / `PORTVERSION`).
**M1 FEITO** no git (`2026-08-14`; GUI/helper via `layer7d --fingerprint`;
`LAYER7_TEST_HW_ID` só com `LAYER7_TEST_ROOT`; sem deploy / `PORTVERSION`).
...
**P2-2 FEITO** no git (`2026-08-14`; CSRF admin fail-closed `Origin` /
`Sec-Fetch-Site`; sem deploy / `PORTVERSION`).
**P2-13 AVALIADO** no git (`2026-08-14`; meia-noite local / DST / UTC
sem correção única segura; sem mudança de runtime).
...
no backend; sem compose/healthcheck; sem deploy / `PORTVERSION`).
**P2-6 Bloco B FEITO** no git (`2026-08-14`; `pg_isready` + `depends_on`
`service_healthy`; sem Docker build/up; sem deploy / `PORTVERSION`).
**P0-2 residual single-use/bind FEITO** no git (`2026-08-14`; `jti` +
`admin_totp_challenges` + consumo transaccional; sem deploy / `PORTVERSION`).
**P3-1 FEITO** no git (`2026-08-14`; sessão única atómica com lock do admin;
sem deploy / `PORTVERSION`).
**P3-2 FEITO** no git (`2026-08-14`; `GET /api/auth/session` inclui
`a.totp_enabled`; sem deploy / `PORTVERSION`).
**P3-3A FEITO** no git (`2026-08-14`; `/login` disabled e inexistente
partilham 401 genérico + bcrypt + `registerLoginFailure`; sem deploy /
`PORTVERSION`).
**P3-3B** FEITO no git (`2026-08-14`;
`POST`/`PUT /api/users` exige password >=12; `/login` não rejeita 10;
sem deploy / `PORTVERSION`).
**P3-3C** FEITO no git (`2026-08-14`;
`verifyTotp` Buffer UTF-8 + guarda de comprimento + `timingSafeEqual`;
sem deploy / `PORTVERSION`).
...
- **Plano (arquivo 【FECHADO】 P0–J):**
  [`docs/archive/planos-fechados/plano-fecho-producao-e-consolidacao.md`](docs/archive/planos-fechados/plano-fecho-producao-e-consolidacao.md)
  ([stub](docs/02-roadmap/plano-fecho-producao-e-consolidacao.md))
...
- Onda: H **PASS** (F6 H1–H5); I **PASS**; J **PASS** (R1–R12 com excepções ADR-0022/0023)
- Candidato lab: 1.9.0
...
- F7/BG-028: **Fase 1** (ADR-0023; `v1.9.58` F1.2); BG-017 checklist PASS
...
- **Plano (arquivo 【FECHADO】):** [`docs/archive/planos-fechados/plano-ipv6-completo.md`](docs/archive/planos-fechados/plano-ipv6-completo.md) ([stub](docs/02-roadmap/plano-ipv6-completo.md))
...
- **P4 retry evidência:** [`docs/tests/evidence/20260813T170000Z-p4-retry-254/`](docs/tests/evidence/20260813T170000Z-p4-retry-254/) — **CLOSED FAIL** (`health_ssh_fail` sample=14)
- **Pós-fail verify:** [`docs/tests/evidence/20260813T223009Z-p4-postfail-verify-254/`](docs/tests/evidence/20260813T223009Z-p4-postfail-verify-254/) — MITM **OFF** (`223009Z`)
...
- **P4 evidência:** [`docs/tests/evidence/20260809T234042Z-p4-soak-254/`](docs/tests/evidence/20260809T234042Z-p4-soak-254/) — **CLOSED FAIL/ABORT** (supervisor nao armado; rollback limpo; Phase C interna tinha PASS)
...
**`30.15` FECHADO** (`20260812T020331Z`): alerta multi-appliance no dashboard
(fase 1 = **só alerta**; decisão 7 — sem `max_activations`); rebind autorizado
...
GA6.3/6.4 **PASS**; candidato **`1.9.58`** publicado (`v1.9.58` / BG-028 Fase 1;
**sem** `.254` / GA5.9 neste bloco);
evidência fecho
...
+ residual campo (**BG-028** Fase 1 — **fechado** `20260813T154800Z`); GA6.6 **PASS**; evidência fecho
...
**Ciclo evidência operacional ABERTO** (`2026-08-14`): **BG-127** — 1ª campanha
**PARTIAL** `20260814T051611Z`; continuação **PARTIAL** `20260814T053905Z`
(GA2.7 **PASS** N2 com licença teste id 14; GA5.9 **FAIL campo** — API então
pré-30.13 rejeitava `nonce`; N3 manteve `valid=1`; legado sem nonce = `409
revoked`; id 13 intocado; produção restaurada). MITM OFF. Evidência
[`20260814T051611Z-bg127`](docs/tests/evidence/20260814T051611Z-bg127/) +
[`20260814T053905Z-bg127`](docs/tests/evidence/20260814T053905Z-bg127/).
**API `30.13` deployada no `.244`** `20260814T142739Z` (só `api`; smoke 404+nonce
PASS; legado sem nonce preservado). Evidência
[`20260814T142739Z-30.13-api-244`](docs/tests/evidence/20260814T142739Z-30.13-api-244/).
...
**BG-128** aberto (remediações P0–P3). **P0-2, P1-1, P1-2, P1-3, P1-4, P2-1, allowlist `30.11` e P1-5…P1-8 + P2-12 FEITOS** no git
(`2026-08-14`; `c2b9fdb` + governação após gates; sem deploy / `PORTVERSION`).
**P2-7+P2-8+P2-10 FEITOS** no git (`2026-08-14`; sem deploy / `PORTVERSION`).
**P2-11 FEITO** no git (`2026-08-14`; sem deploy / `PORTVERSION`).
...
- **Arranque (único desta trilha):**
  [`docs/00-overview/START-HERE-antipirataria.md`](docs/00-overview/START-HERE-antipirataria.md)
- **Diagnóstico (achados A-01…A-10):**
  [`docs/01-architecture/modelo-ameacas-antipirataria.md`](docs/01-architecture/modelo-ameacas-antipirataria.md)
...
- **Evidência P3-8 recheck cut 14-08:**
...
- **P2-16 / BG-151 (rollback preferido ≠ tag `pre-30.13`):**
  [`docs/13-runbooks/bloqueio-deploy-integral-head-30.11.md`](docs/13-runbooks/bloqueio-deploy-integral-head-30.11.md)
  + evidência [`docs/tests/evidence/20260814T142739Z-30.13-api-244/`](docs/tests/evidence/20260814T142739Z-30.13-api-244/)
...
- **Evidência BG-127 1ª campanha (PARTIAL):**
  [`docs/tests/evidence/20260814T051611Z-bg127/`](docs/tests/evidence/20260814T051611Z-bg127/)
- **Evidência BG-127 continuação (PARTIAL; GA2.7 PASS / GA5.9 FAIL campo):**
  [`docs/tests/evidence/20260814T053905Z-bg127/`](docs/tests/evidence/20260814T053905Z-bg127/)
- **Evidência deploy API 30.13 no `.244`:**
...
- **EULA agenda:** [`docs/09-blocking/eula-revisao-juridica-30.19.md`](docs/09-blocking/eula-revisao-juridica-30.19.md)
- **Aviso RR-3:** [`docs/06-releases/aviso-releases-antigas-rr3-30.19.md`](docs/06-releases/aviso-releases-antigas-rr3-30.19.md)
...
- **Auditoria `2026-08-14` / freeze deploy:** **BG-128 ABERTO** — P0-1 ACTIVO
  ...
  **P2-3 FEITO** no git (`2026-08-14`; origin `X-Forwarded-Proto $scheme`);
  **P1-9 AVALIADO** no git (`2026-08-14`; residual pós-P2-3 não aberto no HEAD);
  **P2-2 FEITO** no git (`2026-08-14`; CSRF admin fail-closed; sem deploy / `PORTVERSION`);
  **P2-13 AVALIADO** no git (`2026-08-14`; meia-noite / DST / UTC sem correção única; sem runtime);
  **P2-4 FEITO** no git (`2026-08-14`; incremento atómico de `failure_count`; sem deploy / `PORTVERSION`);
  **P2-6 Bloco A FEITO** no git (`2026-08-14`; `.dockerignore` + `USER node` no backend; sem compose/healthcheck; sem deploy / `PORTVERSION`);
  **P2-6 Bloco B FEITO** no git (`2026-08-14`; `pg_isready` + `depends_on` `service_healthy`; sem Docker build/up; sem deploy / `PORTVERSION`);
  **P0-2 residual single-use/bind FEITO** no git (`2026-08-14`; `jti` + `admin_totp_challenges`; sem deploy / `PORTVERSION`);
  **P3-1 FEITO** no git (`2026-08-14`; sessão única atómica com lock do admin; sem deploy / `PORTVERSION`);
  **P3-2 FEITO** no git (`2026-08-14`; `GET /api/auth/session` inclui `a.totp_enabled`; sem deploy / `PORTVERSION`);
...
1. [`docs/00-overview/START-HERE-antipirataria.md`](docs/00-overview/START-HERE-antipirataria.md)
2. `AGENTS.md`
3. `CORTEX.md` (secção *Trilha Anti-pirataria / Anti-tamper*)
4. [`docs/01-architecture/modelo-ameacas-antipirataria.md`](docs/01-architecture/modelo-ameacas-antipirataria.md) (achados A-01…A-10)
5. [`docs/02-roadmap/plano-antipirataria-anti-tamper.md`](docs/02-roadmap/plano-antipirataria-anti-tamper.md)
   (§0 passo actual, §0.0 R-A…R-L, §0.1 RR-1…RR-5, §1 N1–N8, **§8 Composer**)
6. [`docs/09-blocking/plano-gates-antipirataria.md`](docs/09-blocking/plano-gates-antipirataria.md)
7. ADRs 0030–0033 (`Aceito`) + ficha GO
   `docs/09-blocking/decisoes-humanas-30.1.md` + ADR-0021 + ADR-0025 (coordenar `30.7`)
   + `docs/08-lab/builder-freebsd.md` (SoT pubkey; fluxo verify pós-`30.2`)
...
4. Se histórico IPv6: [`docs/archive/planos-fechados/plano-ipv6-completo.md`](docs/archive/planos-fechados/plano-ipv6-completo.md)
5. Se histórico fecho: [`docs/archive/planos-fechados/plano-fecho-producao-e-consolidacao.md`](docs/archive/planos-fechados/plano-fecho-producao-e-consolidacao.md)

### Para a trilha IPv6 (detalhe técnico)

1. `CORTEX.md` (secção Trilha IPv6)
2. [`docs/00-overview/START-HERE-fecho-producao.md`](docs/00-overview/START-HERE-fecho-producao.md)
3. [`docs/archive/planos-fechados/plano-ipv6-completo.md`](docs/archive/planos-fechados/plano-ipv6-completo.md)
...
- Produto: Layer7 para pfSense CE — **PRONTO PARA ENFORCE** (excepções ADR-0022 CE; ADR-0023 BG-028 **fase 1** em `v1.9.58`)
...
- Evidência P4: docs/tests/evidence/20260809T234042Z-p4-soak-254/ (**CLOSED FAIL/ABORT**; supervisor nao armado; rollback limpo)
- Evidência P4 retry: docs/tests/evidence/20260813T170000Z-p4-retry-254/ (**CLOSED FAIL**; health_ssh_fail sample=14)
- Evidência pós-fail: docs/tests/evidence/20260813T223009Z-p4-postfail-verify-254/ (MITM OFF)
