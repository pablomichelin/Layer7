...
ev = repo / "docs/tests/evidence/20260809T234042Z-p4-soak-254"
verdict = "FAIL"
...
git add \
  ...
  docs/tests/evidence/20260809T234042Z-p4-soak-254/ 2>/dev/null || true
...
repo = Path("/Users/pablomichelin/Documents/Layer 7")
verdict = "FAIL"
for line in (repo/"docs/tests/evidence/20260809T234042Z-p4-soak-254/11-VERDICT.txt").read_text().splitlines():
    if line.startswith("P4_SOAK_VERDICT="):
...
git add -A docs/tests/evidence/20260809T234042Z-p4-soak-254/ \
  CORTEX.md \
  docs/00-overview/START-HERE-identity-mitm.md \
  docs/02-roadmap/checklist-mestre.md \
  docs/02-roadmap/backlog.md \
...
docs(mitm): P4 soak CLOSED ${VERDICT}

Fecha janela lab 4h: health/auditoria, rollback completo e veredicto
honesto. MITM não permanece ON. P5 externo continua proibido.
EOF
)" || echo "COMMIT_SKIP_OR_FAIL"
