...
# Segredos: p4-lib-auth.sh apenas. Não reativa mitm.enabled.
set -u
EV="${EV:-/Users/pablomichelin/Documents/Layer 7/docs/tests/evidence/20260809T234042Z-p4-soak-254}"
...
echo "auth_policy=no_literal_secrets; key_or_0600_tmp_or_SSHPASS_env" | tee -a "$EV/90-rollback-full-start.txt"

REASON_PHP=$(printf '%s' "$REASON" | sed "s/'/'\\\\''/g")
p4_ssh_254 "bash -s" <<R254 | tee "$EV/90-rollback-254.txt"
set +e
...
'
# MONITOR do produto Layer7 — NUNCA repor mitm.enabled=true
php -r '
require_once("config.inc");
require_once("/usr/local/pkg/layer7.inc");
\$data = layer7_load_or_default();
...
/* defesa: garantir mitm OFF mesmo se estado residual */
...
# Windows OpenSSH: prefer EncodedCommand (heredoc stdin frequentemente vazio)
B64_24=$(python3 - <<'PY'
import base64
ps = r'''
...
# Redigir contas/usernames acidentais na evidência
...
p4_ssh_254 "bash -s" <<'V' | tee "$EV/93-post-state.txt"
set +e
php -r '
require_once("/usr/local/pkg/layer7.inc");
$data = layer7_load_or_default();
