...
**Data:** 2026-08-04  
**Rev.:** 2026-08-05f  
**Estado:** SSOT da trilha IPv6 — núcleo + V5 **FECHADOS** (12.10 DNS + 12.11 HTTP/VIP)
...
| DNS forçado / sinkhole | `rdr inet` + A | `rdr inet6` + AAAA (12.10) | — | V5 |
| Block page NAT | `rdr inet` | `rdr inet6` HTTP/HTTPS (12.11) | ADR-0017 | V5 |
| VIP isenção DNS | IPv4 rdr/view | VIP ACL Unbound v6 (12.11) | ADR-0020 | V5 |
| GUI / validação JSON | Dual-stack host/CIDR | Dual-stack + portal6 | — | V4–V5 |
...
|----|----------|--------|-------------|-------------|------|-----|-------|
| M-01 | `src/layer7d/capture.c` + `dns_observe.h` | libpcap → parse IP → nDPI | IPv4+IPv6 L3 + métricas AF; **A+AAAA hint; observe independente de flow hash (`1.9.6`)** | pcap local unbound | V2 | BG-080 | CAP |
| M-02 | `src/layer7d/capture_flow_key.h` | Hash fluxo bidireccional | v4+v6 hash (12.4) | — | V2 | BG-080 | CAP |
| M-03 | `src/layer7d/main.c` | flow_decide, DNS hint, PF add | Gates PF `host_enforce_ok` (12.7); dns_cb A/AAAA→enforce; `dns_resolved` DBG | — | V3 | BG-081 | ENF |
...
| M-09 | `src/layer7d/blacklist.c` + Unbound | Sinkhole / bl tables | A+AAAA Unbound (12.10); HTTP portal v6 (12.11) | — | V3–V5 | BG-081/083 | ENF |
...
| M-13 | `package/.../layer7.inc` | `layer7_generate_rdr_rules_*` | `rdr inet` + **`inet6` :53** (12.10) + **HTTP/HTTPS inet6** (12.11) | — | V5 | BG-083 | NAT |
| M-14 | `package/.../layer7.inc` | VIP DNS / Unbound view | IPv4 + **IPv6 ACL** (12.11) | — | V5 | BG-083 | NAT |
...
| S-07 | M-09: 12.10 AAAA + `rdr inet6` :53; 12.11 HTTP inet6 + listen `[::1]:8099` | V5 | Disclosure Diagnostics |
