# Release Notes — 1.8.11_10 (RASCUNHO / PRÉ-PUBLICAÇÃO)
...
**Artefato (port `package/pfSense-pkg-layer7`):** `PORTVERSION=1.8.11`, `PORTREVISION=10`  
**Nome de pacote FreeBSD típico:** `pfSense-pkg-layer7-1.8.11_10.pkg` (ajustar ao nome exacto do build)

**Checksum / manifesto / assinatura (F1.2+):** preencher após o stage de release
(`pfSense-pkg-layer7-1.8.11_10.pkg.sha256`, `release-manifest.v1.txt`, etc.); ver
...
```sh
fetch -o /tmp/install.sh "https://github.com/pablomichelin/pfsense-layer7/releases/download/v1.8.11_10/install.sh" && sh /tmp/install.sh
```
