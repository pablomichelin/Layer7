...
> **Estado actual:** lab/`latest` = **`1.9.68`**; pin enforce = **`1.9.8`**
> (não alinhados). Histórico Onda F (`2026-08-05`) alinhou temporariamente
> em `_69` — apenas contexto; não usar como canal actual.

> **Release `1.9.68` (BG-159 — inglês de Configurações, `2026-08-15`):** canal publico
> `latest` / comandos abaixo → **`1.9.68`**. **Produção enforce** permanece
> **`1.9.8`**. Rollback lab: **`1.9.65`**. Rollback enforce: **`1.9.0`**.

**Versao canal publico / lab (`latest` e comandos abaixo):** `1.9.68`
...
- **SHA256:** `https://github.com/pablomichelin/Layer7/releases/download/v1.9.68/pfSense-pkg-layer7-1.9.68.pkg.sha256`
...
**Versao anterior lab (rollback a partir de `1.9.68`):** `1.9.67`

- **Release:** `https://github.com/pablomichelin/Layer7/releases/tag/v1.9.67`
- **Pacote `.pkg`:** `https://github.com/pablomichelin/Layer7/releases/download/v1.9.67/pfSense-pkg-layer7-1.9.67.pkg`
- **SHA256 esperado:** `0d85bbf00f33147c7655fa0ab091a71b02c4f7dcb4149d877da2b03a2ad1fdb3`
...
```sh
fetch -o /tmp/install.sh https://github.com/pablomichelin/Layer7/releases/download/v1.9.68/install.sh && sh /tmp/install.sh
```

Baixar o `.pkg` directo da versao `1.9.65`:

```sh
fetch -o /tmp/pfSense-pkg-layer7-1.9.68.pkg https://github.com/pablomichelin/Layer7/releases/download/v1.9.68/pfSense-pkg-layer7-1.9.68.pkg
```

Validar checksum:

```sh
fetch -o /tmp/pfSense-pkg-layer7-1.9.68.pkg.sha256 https://github.com/pablomichelin/Layer7/releases/download/v1.9.68/pfSense-pkg-layer7-1.9.68.pkg.sha256 && sha256 -q /tmp/pfSense-pkg-layer7-1.9.68.pkg | tee /tmp/l7-actual.sha256 && cat /tmp/pfSense-pkg-layer7-1.9.68.pkg.sha256
```

Os dois ultimos `cat` devem mostrar o mesmo `sha256`. Esperado:
`0c12cf38f44347ba69af876eea1e0b3cefc22cde07d9e5fd85ae208876ce0d6f`.
...
> **Nesta release (`1.9.68`)** o caminho oficial e o **`install.sh`**
> carimbado F1.2 (fail-closed). O comando unico manual permanece como
> alternativa. **Canal lab/`latest`:** **`1.9.65`**. **Producao enforce:**
> permanece **`1.9.8`** (GV7.4) ate GO de promocao. Soak `.254`
> = **`1.9.65`** MITM OFF (20.36).
...
```sh
fetch -o /tmp/pfSense-pkg-layer7-1.9.68.pkg https://github.com/pablomichelin/Layer7/releases/download/v1.9.68/pfSense-pkg-layer7-1.9.68.pkg && IGNORE_OSVERSION=yes pkg add -f /tmp/pfSense-pkg-layer7-1.9.68.pkg && sysrc layer7d_enable=YES && service layer7d onestart && layer7d -V
```
...
```sh
IGNORE_OSVERSION=yes pkg add -f /tmp/pfSense-pkg-layer7-1.9.68.pkg
```
...
```sh
service layer7d onestop && fetch -o /tmp/pfSense-pkg-layer7-1.9.68.pkg https://github.com/pablomichelin/Layer7/releases/download/v1.9.68/pfSense-pkg-layer7-1.9.68.pkg && IGNORE_OSVERSION=yes pkg add -f /tmp/pfSense-pkg-layer7-1.9.68.pkg && service layer7d onestart && layer7d -V
```
...
```sh
service layer7d onestop && pkg delete -y pfSense-pkg-layer7 && fetch -o /tmp/pfSense-pkg-layer7-1.9.68.pkg https://github.com/pablomichelin/Layer7/releases/download/v1.9.68/pfSense-pkg-layer7-1.9.68.pkg && IGNORE_OSVERSION=yes pkg add -f /tmp/pfSense-pkg-layer7-1.9.68.pkg && sysrc layer7d_enable=YES && service layer7d onestart
```
...
> **Nesta release (`1.9.68`)** o `uninstall.sh` automatico **e publicado**
...
```sh
fetch -o /tmp/uninstall.sh https://github.com/pablomichelin/Layer7/releases/download/v1.9.68/uninstall.sh
```
...
Para reinstalar a versao actual (`1.9.68`), usar o **`install.sh`** da
