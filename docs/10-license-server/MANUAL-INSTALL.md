...
- **Release:** `https://github.com/pablomichelin/Layer7/releases/tag/v1.5.3`
- **Pacote `.pkg`:** `https://github.com/pablomichelin/Layer7/releases/download/v1.5.3/pfSense-pkg-layer7-1.5.3.pkg`
- **SHA256:** `https://github.com/pablomichelin/Layer7/releases/download/v1.5.3/pfSense-pkg-layer7-1.5.3.pkg.sha256`
...
Instalar explicitamente a versao `1.5.3`:

```sh
fetch -o /tmp/install.sh https://raw.githubusercontent.com/pablomichelin/Layer7/main/install.sh && sh /tmp/install.sh --version 1.5.3
```

Baixar o `.pkg` directo da versao `1.5.3`:

```sh
fetch -o /tmp/pfSense-pkg-layer7-1.5.3.pkg https://github.com/pablomichelin/Layer7/releases/download/v1.5.3/pfSense-pkg-layer7-1.5.3.pkg
```
...
Para uma versao especifica: `sh /tmp/install.sh --version 1.5.3`

**Comando unico manual (Command Prompt):**

```sh
fetch -o /tmp/pfSense-pkg-layer7-1.5.3.pkg https://github.com/pablomichelin/Layer7/releases/download/v1.5.3/pfSense-pkg-layer7-1.5.3.pkg && IGNORE_OSVERSION=yes pkg add -f /tmp/pfSense-pkg-layer7-1.5.3.pkg && sysrc layer7d_enable=YES && service layer7d onestart && layer7d -V
```
...
```
```sh
IGNORE_OSVERSION=yes pkg add -f /tmp/pfSense-pkg-layer7-1.5.3.pkg
```
...
```sh
service layer7d onestop && fetch -o /tmp/pfSense-pkg-layer7-1.5.3.pkg https://github.com/pablomichelin/Layer7/releases/download/v1.5.3/pfSense-pkg-layer7-1.5.3.pkg && IGNORE_OSVERSION=yes pkg add -f /tmp/pfSense-pkg-layer7-1.5.3.pkg && service layer7d onestart && layer7d -V
```
...
```sh
service layer7d onestop && pkg delete -y pfSense-pkg-layer7 && IGNORE_OSVERSION=yes pkg add -f /tmp/pfSense-pkg-layer7-1.5.3.pkg && sysrc layer7d_enable=YES && service layer7d onestart
```
...
Blacklists UT1 + categorias personalizadas (v1.5.3+):
