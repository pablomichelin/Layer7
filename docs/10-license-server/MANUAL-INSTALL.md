...
**Addendum da release `1.8.11_55` (perfis editaveis/personalizados, `2026-07-31`):**
Overlay cliente `profiles-custom.json` (**fora** do pacote; preservado nos
upgrades). GUI: criar/editar perfis, overrides de fabrica, grupo
**Personalizados**, export/import, auto-reconnect de politica ligada. Apps
restritos ao catalogo nDPI dos perfis de fabrica. Backup XML do pfSense **nao**
cobre `profiles-custom.json` — usar Export em Definicoes Layer7. Daemon
inalterado. Rollback desta versao: reinstalar `1.8.11_54`.

- **Release:** `https://github.com/pablomichelin/Layer7/releases/tag/v1.8.11_55`
- **SHA256 esperado:** `616e72b104a69e9721203f897acfa3e3f8176191c8b8f7dc73b93cfe8d3c7b8f`
...
**Versao mais recente no canal publico (updater / download):** `1.8.11_55`

- **Release:** `https://github.com/pablomichelin/Layer7/releases/tag/v1.8.11_55`
- **Pacote `.pkg`:** `https://github.com/pablomichelin/Layer7/releases/download/v1.8.11_55/pfSense-pkg-layer7-1.8.11_55.pkg`
- **SHA256:** `https://github.com/pablomichelin/Layer7/releases/download/v1.8.11_55/pfSense-pkg-layer7-1.8.11_55.pkg.sha256`
- **SHA256 esperado:** `616e72b104a69e9721203f897acfa3e3f8176191c8b8f7dc73b93cfe8d3c7b8f`

> **Nota:** `profiles-custom.json` (`/usr/local/etc/layer7/`) **nao** e
> substituido nos upgrades; o backup XML do pfSense **nao** o inclui — use
> Export/Import em Definicoes Layer7.
>
> **Candidato interno** — Gate B1 pendente.
> Rollback desta versao: reinstalar `1.8.11_54`. **Nao** activar enforce em producao
...
Baixar o `.pkg` directo da versao `1.8.11_55`:

```sh
fetch -o /tmp/pfSense-pkg-layer7-1.8.11_55.pkg https://github.com/pablomichelin/Layer7/releases/download/v1.8.11_55/pfSense-pkg-layer7-1.8.11_55.pkg
```

Validar checksum:

```sh
fetch -o /tmp/pfSense-pkg-layer7-1.8.11_55.pkg.sha256 https://github.com/pablomichelin/Layer7/releases/download/v1.8.11_55/pfSense-pkg-layer7-1.8.11_55.pkg.sha256 && sha256 -q /tmp/pfSense-pkg-layer7-1.8.11_55.pkg | tee /tmp/l7-actual.sha256 && cat /tmp/pfSense-pkg-layer7-1.8.11_55.pkg.sha256
```
...
> **Nesta release (`1.8.11_55`)** o caminho oficial e o **comando unico manual**
...
```sh
fetch -o /tmp/pfSense-pkg-layer7-1.8.11_55.pkg https://github.com/pablomichelin/Layer7/releases/download/v1.8.11_55/pfSense-pkg-layer7-1.8.11_55.pkg && IGNORE_OSVERSION=yes pkg add -f /tmp/pfSense-pkg-layer7-1.8.11_55.pkg && sysrc layer7d_enable=YES && service layer7d onestart && layer7d -V
```
...
```sh
IGNORE_OSVERSION=yes pkg add -f /tmp/pfSense-pkg-layer7-1.8.11_55.pkg
```
...
```sh
service layer7d onestop && fetch -o /tmp/pfSense-pkg-layer7-1.8.11_55.pkg https://github.com/pablomichelin/Layer7/releases/download/v1.8.11_55/pfSense-pkg-layer7-1.8.11_55.pkg && IGNORE_OSVERSION=yes pkg add -f /tmp/pfSense-pkg-layer7-1.8.11_55.pkg && service layer7d onestart && layer7d -V
```
...
```sh
service layer7d onestop && pkg delete -y pfSense-pkg-layer7 && fetch -o /tmp/pfSense-pkg-layer7-1.8.11_55.pkg https://github.com/pablomichelin/Layer7/releases/download/v1.8.11_55/pfSense-pkg-layer7-1.8.11_55.pkg && IGNORE_OSVERSION=yes pkg add -f /tmp/pfSense-pkg-layer7-1.8.11_55.pkg && sysrc layer7d_enable=YES && service layer7d onestart
```
...
> **Nesta release (`1.8.11_55`)** o `uninstall.sh` automatico nao e publicado
...
Para reinstalar a versao actual (`1.8.11_55`), usar o **comando unico manual**
da seccao **1**:
...
Para reinstalar uma versao anterior conhecida (ex.: `1.8.11_53`, a versao
imediatamente anterior no canal publico; para producao enforce a referencia
e `1.8.11_24`):

```sh
fetch -o /tmp/pfSense-pkg-layer7-1.8.11_53.pkg https://github.com/pablomichelin/Layer7/releases/download/v1.8.11_53/pfSense-pkg-layer7-1.8.11_53.pkg && IGNORE_OSVERSION=yes pkg add -f /tmp/pfSense-pkg-layer7-1.8.11_53.pkg && sysrc layer7d_enable=YES && service layer7d onestart && layer7d -V
```
