...
> **Estado actual:** lab/`latest` = **`1.9.40`**; pin enforce = **`1.9.8`**
> (não alinhados). Histórico Onda F (`2026-08-05`) alinhou temporariamente
> em `_69` — apenas contexto; não usar como canal actual.

> **Release `1.9.40` (MITM 20.10b, `2026-08-09`):** canal publico `latest` /
> comandos abaixo → **`1.9.40`**. **Produção enforce** permanece **`1.9.8`**.
> Rollback lab: **`1.9.39`**. Rollback enforce: **`1.9.0`**.

**Versao canal publico / lab (`latest` e comandos abaixo):** `1.9.40`

- **Release:** `https://github.com/pablomichelin/Layer7/releases/tag/v1.9.40`
- **Pacote `.pkg`:** `https://github.com/pablomichelin/Layer7/releases/download/v1.9.40/pfSense-pkg-layer7-1.9.40.pkg`
- **SHA256:** `https://github.com/pablomichelin/Layer7/releases/download/v1.9.40/pfSense-pkg-layer7-1.9.40.pkg.sha256`
- **SHA256 esperado:** `fbbf206d1b159722a28073dd402f9b0c8ef381eff07eb3a886e5ef8310a41afe`
...
Baixar o `.pkg` directo da versao `1.9.40`:

```sh
fetch -o /tmp/pfSense-pkg-layer7-1.9.40.pkg https://github.com/pablomichelin/Layer7/releases/download/v1.9.40/pfSense-pkg-layer7-1.9.40.pkg
```

Validar checksum:

```sh
fetch -o /tmp/pfSense-pkg-layer7-1.9.40.pkg.sha256 https://github.com/pablomichelin/Layer7/releases/download/v1.9.40/pfSense-pkg-layer7-1.9.40.pkg.sha256 && sha256 -q /tmp/pfSense-pkg-layer7-1.9.40.pkg | tee /tmp/l7-actual.sha256 && cat /tmp/pfSense-pkg-layer7-1.9.40.pkg.sha256
```
...
> **Nesta release (`1.9.40`)** o caminho oficial e o **comando unico manual**
...
> backlog. **Canal lab/`latest`:** **`1.9.40`**. **Producao enforce:**
> permanece **`1.9.8`** (GV7.4) ate GO de promocao.

**Comando unico manual (recomendado — uma linha, Command Prompt ou SSH):**

```sh
fetch -o /tmp/pfSense-pkg-layer7-1.9.40.pkg https://github.com/pablomichelin/Layer7/releases/download/v1.9.40/pfSense-pkg-layer7-1.9.40.pkg && IGNORE_OSVERSION=yes pkg add -f /tmp/pfSense-pkg-layer7-1.9.40.pkg && sysrc layer7d_enable=YES && service layer7d onestart && layer7d -V
```
...
```sh
IGNORE_OSVERSION=yes pkg add -f /tmp/pfSense-pkg-layer7-1.9.40.pkg
```
...
```sh
service layer7d onestop && fetch -o /tmp/pfSense-pkg-layer7-1.9.40.pkg https://github.com/pablomichelin/Layer7/releases/download/v1.9.40/pfSense-pkg-layer7-1.9.40.pkg && IGNORE_OSVERSION=yes pkg add -f /tmp/pfSense-pkg-layer7-1.9.40.pkg && service layer7d onestart && layer7d -V
```
...
```sh
service layer7d onestop && pkg delete -y pfSense-pkg-layer7 && fetch -o /tmp/pfSense-pkg-layer7-1.9.40.pkg https://github.com/pablomichelin/Layer7/releases/download/v1.9.40/pfSense-pkg-layer7-1.9.40.pkg && IGNORE_OSVERSION=yes pkg add -f /tmp/pfSense-pkg-layer7-1.9.40.pkg && sysrc layer7d_enable=YES && service layer7d onestart
```
...
> **Nesta release (`1.9.40`)** o `uninstall.sh` automatico nao e publicado
