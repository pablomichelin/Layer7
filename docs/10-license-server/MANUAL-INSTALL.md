...
> **Release `1.9.11` (TOCTOU + DNS corr, `2026-08-05`):** canal publico `latest` /
> comandos abaixo → **`1.9.11`**. **Produção enforce** permanece **`1.9.8`**
> (GV7.4) até GO. Rollback lab: **`1.9.9`**. Rollback enforce: **`1.9.0`**.

**Versao canal publico / lab (`latest` e comandos abaixo):** `1.9.11`

- **Release:** `https://github.com/pablomichelin/Layer7/releases/tag/v1.9.11`
- **Pacote `.pkg`:** `https://github.com/pablomichelin/Layer7/releases/download/v1.9.11/pfSense-pkg-layer7-1.9.11.pkg`
- **SHA256:** `https://github.com/pablomichelin/Layer7/releases/download/v1.9.11/pfSense-pkg-layer7-1.9.11.pkg.sha256`
- **SHA256 esperado:** `dde1e17b00820d56bbee231087f92a4552da73dc00ea18820d17750ad35cd1be`
- **`releases/latest`:** `https://github.com/pablomichelin/Layer7/releases/latest`

**Versao anterior lab (rollback a partir de `1.9.11`):** `1.9.10`
...
Baixar o `.pkg` directo da versao `1.9.11`:

```sh
fetch -o /tmp/pfSense-pkg-layer7-1.9.11.pkg https://github.com/pablomichelin/Layer7/releases/download/v1.9.11/pfSense-pkg-layer7-1.9.11.pkg
```

Validar checksum:

```sh
fetch -o /tmp/pfSense-pkg-layer7-1.9.11.pkg.sha256 https://github.com/pablomichelin/Layer7/releases/download/v1.9.11/pfSense-pkg-layer7-1.9.11.pkg.sha256 && sha256 -q /tmp/pfSense-pkg-layer7-1.9.11.pkg | tee /tmp/l7-actual.sha256 && cat /tmp/pfSense-pkg-layer7-1.9.11.pkg.sha256
```

Os dois ultimos `cat` devem mostrar o mesmo `sha256`. Esperado:
`dde1e17b00820d56bbee231087f92a4552da73dc00ea18820d17750ad35cd1be`.
...
> **Nesta release (`1.9.11`)** o caminho oficial e o **comando unico manual**
...
> backlog. **Canal lab/`latest`:** **`1.9.11`**. **Producao enforce:**
> permanece **`1.9.8`** (GV7.4) ate GO de promocao.

**Comando unico manual (recomendado — uma linha, Command Prompt ou SSH):**

```sh
fetch -o /tmp/pfSense-pkg-layer7-1.9.11.pkg https://github.com/pablomichelin/Layer7/releases/download/v1.9.11/pfSense-pkg-layer7-1.9.11.pkg && IGNORE_OSVERSION=yes pkg add -f /tmp/pfSense-pkg-layer7-1.9.11.pkg && sysrc layer7d_enable=YES && service layer7d onestart && layer7d -V
```
...
```sh
IGNORE_OSVERSION=yes pkg add -f /tmp/pfSense-pkg-layer7-1.9.11.pkg
```
...
```sh
service layer7d onestop && fetch -o /tmp/pfSense-pkg-layer7-1.9.11.pkg https://github.com/pablomichelin/Layer7/releases/download/v1.9.11/pfSense-pkg-layer7-1.9.11.pkg && IGNORE_OSVERSION=yes pkg add -f /tmp/pfSense-pkg-layer7-1.9.11.pkg && service layer7d onestart && layer7d -V
```
...
```sh
service layer7d onestop && pkg delete -y pfSense-pkg-layer7 && fetch -o /tmp/pfSense-pkg-layer7-1.9.11.pkg https://github.com/pablomichelin/Layer7/releases/download/v1.9.11/pfSense-pkg-layer7-1.9.11.pkg && IGNORE_OSVERSION=yes pkg add -f /tmp/pfSense-pkg-layer7-1.9.11.pkg && sysrc layer7d_enable=YES && service layer7d onestart
```
...
> **Nesta release (`1.9.11`)** o `uninstall.sh` automatico nao e publicado
...
Para reinstalar a versao actual (`1.9.11`), usar o **comando unico manual**
da seccao **1**:
