...
| **IPv6 completo V0–V6** | **ABERTA** — passo **12.7** (V3) | [`plano-ipv6-completo.md`](../02-roadmap/plano-ipv6-completo.md) |
...
| **Passo autorizado** | **12.7** (Onda V3 — `enforce.c`/`main.c` PF tabelas + kill states v6) |
| **BG activo** | BG-081 (V3; 12.6 policy CIDR concluído) |
...
| Passo **12.7** (autorizado) | `enforce.c`/`main.c` PF tabelas + kill states v6 — **V3** |
...
Regras absolutas:
- Executar SOMENTE o passo 12.x autorizado no CORTEX (agora: 12.7).
...
| **12.7** | V3 | `enforce.c`/`main.c` PF tabelas + kill states v6 — **actual** |
| 12.8 | V3 | Allowlist v6 |
