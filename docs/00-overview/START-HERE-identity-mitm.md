...
| Campo | Valor |
|-------|-------|
| Plano | **ABERTO** — Identity + MITM Add-on (rev. `2026-08-08c`) |
...
| Passo actual | **IM6 / 20.24** — match/enforce `ad_*` via mapa (20.23 parse/GUI **PASS**) |
| Código do produto nesta trilha | **20.22 PASS** (audit conflict/last-writer); 20.21 normalize; … |
| Rev. do plano | **`2026-08-08c`** |
...
Tarefa: continuar no passo actual (IM6 / 20.24 — match/enforce ad_* via mapa; 20.23 PASS).
```
...
- Passo actual: IM6 / 20.24 (match/enforce ad_* via mapa)
...
- Candidato: **1.9.27** (publicado lab/`latest`)
```
