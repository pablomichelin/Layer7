...
| `10-RUNBOOK-OPERACIONAL-E-ROLLBACK.md` | operacao e rollback | runbook expandido legado | Historico | complementar a `docs/13-runbooks/` e `MANUAL-INSTALL.md` | preservar | F6 |
...
| `docs/00-overview/f6-plano-higiene-estrutural-residual.md` | plano F6 residual | gate G0–G7 + exclusões + lotes P1–P4 (BG-112) | Canónico | pós-H5 | actualizar ao executar/DEFER lotes | F6 residual |
| `docs/00-overview/f6-inventario-higiene-estrutural-2026-08-09.md` | inventário higiene | INV-* rastreáveis | Canónico (auditoria) | — | não apagar; actualiza se novo resíduo | F6 residual |
| `docs/00-overview/f6-classificacao-candidatos-higiene-2026-08-09.md` | classificação higiene | MANTER/ARQUIVAR/REMOVER/CORRIGIR | Canónico (auditoria) | inventário | execução só com GO + gate | F6 residual |
...
| `docs/00-overview/frontend-redesign-analise.md` | redesign frontend BG-174 | diagnóstico, arquitectura de informação, alternativas, componentes e decisões humanas | Canónico | governa a proposta; complementa PRD e GUI conceptual | manter vivo durante GUI0–GUI7 | F4/F5/F7 |
| `docs/00-overview/frontend-redesign-wireframes.md` | wireframes frontend BG-174 | baixa fidelidade e estados de aceitação para dez superfícies prioritárias | Canónico | concretiza a análise sem substituir a matriz funcional | rever por gate visual antes de cada onda | F4/F5/F7 |
| `docs/01-architecture/frontend-redesign-inventario-paridade.md` | inventário e paridade frontend | SSOT de rotas, campos, efeitos, gates e destino proposto | Canónico | impede perda silenciosa de funções; complementa código e GUI real | actualizar no mesmo bloco de cada mudança GUI | F4/F5/F7 |
| `docs/02-roadmap/plano-redesign-frontend.md` | execução frontend BG-174 | plano reversível GUI0–GUI7, gates, testes e rollback | Canónico | governa a execução; não reabre Identity+MITM | avançar só com GO humano e gate da onda | F4/F5/F7 |
...
| `docs/13-runbooks/README.md` | indice de runbooks | agregador operacional local | Suplementar | complementar ao `MANUAL-INSTALL.md` | manter leve | F4/F7 |
| `docs/13-runbooks/license-server-publicacao-segura.md` | publicacao segura do license server | runbook operativo da F2.1 para edge proxy, TLS, origin privado e validacoes de exposicao | Canónico | complementar ao ADR-0007 e ao `MANUAL-USO-LICENCAS.md` | manter vivo enquanto a F2/F7 exigirem governanca da borda | F2/F7 |
| `docs/13-runbooks/license-server-auth-sessao.md` | auth/sessao do license server | runbook operativo da F2.2 para login, sessao administrativa, expiracao, logout e troubleshooting | Canónico | complementar ao ADR-0008 e ao `MANUAL-USO-LICENCAS.md` | manter vivo enquanto a F2/F7 exigirem governanca da superficie administrativa | F2/F7 |
| `docs/13-runbooks/license-server-segredos-bootstrap.md` | segredos e bootstrap do license server | runbook operativo da F2.5 para ownership minimo, segredo bootstrap e recuperacao de password administrativa | Canónico | complementar aos ADRs da F2 e ao `MANUAL-USO-LICENCAS.md` | manter vivo enquanto a operacao do server exigir custodia clara de segredos | F2/F7 |
| `docs/13-runbooks/license-server-backup-restore.md` | backup/restore do license server | runbook operativo da F2.5 para dump SQL, restore controlado e recuperacao minima do PostgreSQL | Canónico | complementar ao runbook de segredos/bootstrap e ao `MANUAL-USO-LICENCAS.md` | manter vivo enquanto a operacao do server exigir recuperacao basica | F2/F7 |
| `docs/13-runbooks/f3-11-live-access-checklist.md` | checklist live F3.11 | runbook operativo canonico para validar acessos, host, schema, admin, appliance e inventario antes de nova readiness/campanha da F3.11 | Canónico | novo | manter vivo enquanto a F3.11 permanecer bloqueada ou em preparacao | F3 |
| `docs/13-runbooks/f3-11-input-triage-runbook.md` | triagem de entrega F3.11 | runbook operativo canonico para receber, validar, aceitar ou rejeitar cada insumo externo antes da readiness | Canónico | novo | manter vivo enquanto a F3.11 permanecer bloqueada ou em preparacao | F3 |
| `docs/13-runbooks/f3-11-evidence-intake-template.md` | intake de evidencias F3.11 | template canónico preenchivel para registar recepcao, validacao e conclusao de cada insumo externo | Canónico | novo | manter vivo enquanto a F3.11 permanecer bloqueada ou em preparacao | F3 |
| `docs/13-runbooks/f3-11-cycle-report-template.md` | ciclo operacional F3.11 | template canónico da rodada completa de recepcao, triagem, actualizacao de blockers e decisao final da F3.11 | Canónico | novo | manter vivo enquanto a F3.11 permanecer bloqueada ou em preparacao | F3 |
| `scripts/license-validation/run-pfsense-gui-license-flow.sh` | helper GUI pfSense F3 | helper canónico para login GUI autenticado, prova de `layer7_settings.php` e submissao legitima de `register_license` / `revoke_license` com evidencias por `run_id`, incluindo execucao via `--ssh-target` quando a GUI relevante so estiver acessivel no loopback do appliance | Canónico | novo | manter vivo enquanto o `DR-05` depender da trilha GUI autenticada | F3 |
...
| `docs/09-blocking/README.md` | trilha de bloqueio | índice da área (actualizado `2026-08-14`) | Suplementar (índice) | aponta para reconciliação, gates e auditoria 2026-08-14 | manter com SSOT da área | F4+ |
| `docs/09-blocking/auditoria-licencas-auth-deploy-2026-08-14.md` | auditoria licenças/auth/deploy | **SSOT** achados P0–P3 + bloqueio P0-1 contra deploy integral HEAD | Canónico | complementar a BG-128 e ao runbook de freeze | serving `30.11` versionado; freeze **não** encerrado; não tratar compose HEAD como live | manutenção |
| `docs/09-blocking/nota-404-esperado-cut-30.11.md` | 404 esperado pós-cut `30.11` | **SSOT** P3-9 opção A — 404 anónimo do espelho é contrato; URLs no cliente mantidos | Canónico | complementar a prep-cut / P3-8 / GA4.11 | não reupload sem GO; não remover URLs sem bloco próprio | manutenção |
| `docs/13-runbooks/bloqueio-deploy-integral-head-30.11.md` | freeze deploy HEAD↔30.11 | runbook operativo **P0-1 ACTIVO** (serving versionado; freeze **não** encerrado) | Canónico | complementar à auditoria e à publicação segura | vivo até GO do primeiro rebuild `api` + smoke | manutenção |
...
| `docs/04-package/gui-validation.md` | validacao GUI | material auxiliar de validacao | Suplementar | complementar a package docs | rever na F5 | F5 |
...
| `scripts/package/README.md` | scripts do pacote | indice check/smoke; ordem canónica com `make package` | Suplementar | complementar a docs package/tests e `builder-freebsd` | rever na F5/F7 | F5/F7 |
...
| `src/layer7d/README.md` | daemon source | build local; smoke; nota ordem check/smoke/`make package` do port | Suplementar | complementar a docs core/daemon | rever na F4/F6 | F4/F6 |
