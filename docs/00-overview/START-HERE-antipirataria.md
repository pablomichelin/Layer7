# START HERE — Anti-pirataria e Anti-tamper 【`30.10` FECHADO · `30.11` preflight auth PASS · cut **PENDENTE** · **Composer 2.5**】
...
> **Cut do espelho:** **não executado** — exige GO gestor + GA4.12 emitida.
> **Próximo passo:** **`30.11` cut** (retirada espelho público corrente) — **GO humano**.
...
6. **Não** executar o **cut** de `30.11` (espelho) sem **GO humano** explícito (RR-1).
...
| [`plano-gates-antipirataria.md`](../09-blocking/plano-gates-antipirataria.md) | Gates — GA4.4 **PASS**; primary auth preflight **PASS**; falta cut `30.11` |
...
| Próxima acção | **`30.11` cut** (GO gestor + emitir GA4.12 antes) |
| Gate activo | **GA4 parcial** — GA4.4–4.7/4.9 PASS; primary auth OK; falta GA4.10/12/15 |
...
30.9 live PASS; GA4.4 PASS; primary auth GET PASS (20260812T003214Z);
próximo = cut 30.11 (retirada espelho — RR-1; GA4.12 antes).
...
Evidência auth: docs/tests/evidence/20260812T003214Z-30.11-auth-get-254/
AGORA (só com GO humano de cut): executar retirada do espelho — um passo.
...
| AP1 | GA2 | **PARCIAL** |
| AP1 | GA3 | **PASS** |
| AP2 | GA4 | **PARCIAL** — GA4.4–4.7/4.9 **PASS**; primary auth **PASS**; falta cut (GA4.10/12/15) |
...
- Gate: GA4.4–4.7/4.9 PASS; falta GA4.10/12/15 (cut + coms)
...
| Coms GA4.12 (rascunho) | [`../13-runbooks/content-mirror-comms-ga4.12-draft.md`](../13-runbooks/content-mirror-comms-ga4.12-draft.md) |
| Contrato 30.8 | [`../01-architecture/contrato-token-subscricao-conteudo-30.8.md`](../01-architecture/contrato-token-subscricao-conteudo-30.8.md) |
