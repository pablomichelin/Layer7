# START HERE — Anti-pirataria e Anti-tamper 【`30.6` FECHADO · próximo `30.7` · **Composer 2.5**】

> **Diagnóstico ACEITE** `2026-08-10` — [`modelo-ameacas-antipirataria.md`](../01-architecture/modelo-ameacas-antipirataria.md).
> **`30.6` FECHADO** — anti-rollback de relógio; GA3 **PASS** (GA3.7 DEFERRED); BG-116 **Concluido**.
> **Passo actual a executar: `30.7`** — entitlements assinados para a GUI (BG-120).
...
> **Evidência 30.6:** [`../tests/evidence/20260810T201043Z-30.6-anti-rollback/`](../tests/evidence/20260810T201043Z-30.6-anti-rollback/)
> **Canal lab/`latest`:** `1.9.51` (publicado). Rollback lab: `1.9.50`.
...
1. Colar **apenas** o caminho acima **ou** o *Prompt — 30.7* abaixo.
...
| [`plano-gates-antipirataria.md`](../09-blocking/plano-gates-antipirataria.md) | Gates — GA0/GA1/GA3 **PASS**; GA2 parcial |
...
| Passo actual | **`30.6` FECHADO** — próximo a executar: **`30.7`** |
| Decisão 4 (pubkey) | **Fora do git (builder)** — SoT aplicada; **inalterada** em 30.6 |
| Gate activo | **GA2 parcial** — faltam GA2.6–2.10 com `30.7`; **GA3 PASS** |
| Código de produto | **`30.6` aplicado** (`license.c` + clock-mark); BG-116 Concluido |
| Agente | **Composer 2.5** — um passo por chat (plano §8) |
| Canal lab/`latest` | **`1.9.51`** — rollback lab **`1.9.50`** |
...
5. [`plano-antipirataria-anti-tamper.md`](../02-roadmap/plano-antipirataria-anti-tamper.md) — §0, §0.0, §0.1 RR, §1 N1–N8, **§8 Composer**, passo actual (`30.7`)
6. [`plano-gates-antipirataria.md`](../09-blocking/plano-gates-antipirataria.md) — GA2.8–2.10
7. [ADR-0030](../03-adr/ADR-0030-postura-anti-tamper-layer7d.md) (entitlements) + ADR-0025
8. Evidência `30.6`: [`../tests/evidence/20260810T201043Z-30.6-anti-rollback/`](../tests/evidence/20260810T201043Z-30.6-anti-rollback/)

Baseline: [`ESTADO-PRODUTO-E-PLANOS-FECHADOS.md`](ESTADO-PRODUTO-E-PLANOS-FECHADOS.md)

---

## Prompt — Composer 2.5 (passo `30.7`)

```text
Modelo: Composer 2.5.
Contexto: trilha Anti-pirataria / Anti-tamper; baseline enforce 1.9.8; lab/latest 1.9.51 (.pkg).
...
Gates: docs/09-blocking/plano-gates-antipirataria.md (GA2.8–2.10)
Ler na ordem do START-HERE; cumprir plano §8.
Estado: 30.6 FECHADO + 1.9.51 publicado; GA3 PASS; executar SOMENTE 30.7.
AGORA: 30.7 — entitlements assinados para a GUI (BG-120); eliminar fallback
sem verificação em layer7_entitlements(); derivar de material já assinado pelo
servidor (.lic / check-in); NÃO assinar localmente com chave embutida; coordenar
ADR-0025; SEM misturar comportamento MITM; NÃO iniciar 30.8 neste chat salvo
pedido explícito.
Proibido: ofuscação; misturar passos; MITM/IPv6/enforce; trocar pubkey;
daemon a assinar com chave própria embutida.
...
| AP1 | GA2 | **PARCIAL** — GA2.1–2.5 + GA2.11 PASS; faltam GA2.6–2.10 (`30.7`/lab) |
...
- Onda: AP1 em curso
- Passo: 30.6 FECHADO; PRÓXIMO EXECUTAR = 30.7
