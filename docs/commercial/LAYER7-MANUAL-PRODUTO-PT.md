...
**Regra:** os comandos abaixo correspondem à release **`1.9.47`**. Em cada
...
| Canal | Versão | Papel |
|-------|--------|--------|
| **`latest` / updater GUI** | **`1.9.47`** | Pacote público actual |
| **Produção enforce (pin estável)** | **`1.9.8`** | Referência de enforce até GO de promoção |
| Rollback lab a partir de `1.9.47` | **`1.9.46`** | Pacote anterior no canal lab |

**MITM (inspecção TLS):**

- Default **OFF** no pacote.
- Só com escopo explícito origem **e** destino (proibido origem aberta/`from any`).
- Janela temporal com auto-desactivação (failsafe) a partir de `1.9.47`.
...
**Não confundir:** ter o pacote `1.9.47` instalado **não** activa MITM nem
...
> Nesta release o caminho oficial é **fetch + `pkg add`**. Scripts
> `install.sh` / `uninstall.sh` **não** estão anexados a `v1.9.47`.
...
### 4.5 Rollback lab (`1.9.47` → `1.9.46`)
...
SHA256 esperado `1.9.46`:
`10998477ef7ae966e6c3566baeb973f922858fc72cc4d3a2dcdd0fb17bae72f5`.

### 4.6 Pin enforce `1.9.8` (produção estável)

Só quando a vossa política for o pin enforce — não é o mesmo que `latest`:

```sh
fetch -o /tmp/pfSense-pkg-layer7-1.9.8.pkg \
  https://github.com/pablomichelin/Layer7/releases/download/v1.9.8/pfSense-pkg-layer7-1.9.8.pkg
```

SHA256 esperado `1.9.8`:
`229639243fc31333251fa286690bf87db9f20b644039b857ca283d16501a99ec`.
...
| Janela | Failsafe / auto-disable (`1.9.47`) |
