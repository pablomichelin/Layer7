...
- [ ] **BG-174 / GUI1 piloto Status** — **candidato `1.9.80` implementado,
  pendente de gates/commit/build/publicação** (`2026-08-31`):
  `layer7_status.php` nativo; handler intacto; `layer7.inc` intocado;
  `PORTVERSION` 1.9.80; SHA256 `PENDING_BUILD`. **`latest` continua
  `1.9.79`**. Gate estático `tests/functional/test_status_native_view.php`.
  GUI2–GUI7 bloqueados até GO; sem tag/build/release.
...
- [x] **BG-174 / ADR-0037 emenda visual** — **FEITO no git** (`c429be3`)
...
- [x] **BG-174 / ADR-0037 emenda frontend-only** — **FEITO no git**
