...
| BG-155 | Correções da revisão de código: lock de check-in, zone-id PF, lockfiles Docker e gates locais/release | Alta | license server + daemon + build | manutenção / F7 | revoke concorrente pode emitir resposta `active`; IP IPv6 com `%iface` chega ao `pfctl`; imagens não reproduzíveis; gates podem falhar por fontes/aceite de licença divergentes | M | Alto | **FEITO no git** (`3762c3c`, `8b70bb9`, `f0cf615`; `2026-08-15`; assinatura/release pendentes) | Check-in serializa `FOR UPDATE` com operações administrativas; zone-id é rejeitado; Docker usa lockfiles + `npm ci`; runner TLS injeta flags Homebrew; smoke inclui o gate de licença e `deployz` não depende de confirmação interativa. Pacote/stage `1.9.64` FreeBSD validado; falta assinatura oficial e publicação. P0-1 continua: não fazer deploy `.244` sem GO. |
