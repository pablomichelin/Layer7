...
| BG-073 | Isencao VIP no caminho DNS (sinkhole Unbound + DNS forcado) | Alta | package/Unbound/PF | Caminho B / F4.3 | VIP na `vip-isentos` continua sujeito a sinkhole global e rdr :53 `from any` | G | Medio | Concluido (Bloco D; `1.8.11_59`) | ADR-0020; opção (a) view Unbound implementada; fallback (b) rdr `from !<layer7_exc_allow_N>`; gate lab Bloco E pendente; producao enforce continua `_24` |
