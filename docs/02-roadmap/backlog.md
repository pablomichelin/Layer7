...
| BG-073 | Isencao VIP no caminho DNS (sinkhole Unbound + DNS forcado) | Alta | package/Unbound/PF | Caminho B / F4.3 | VIP na `vip-isentos` continua sujeito a sinkhole global e rdr :53 `from any` | G | Medio | Feature completa A–E; execucao lab sec. 20 pendente | ADR-0020; opção (a) view Unbound implementada; fallback (b) rdr `from !<layer7_exc_allow_N>`; roteiro «director isento de tudo» sec. 20; producao enforce continua `_24` |
