...
| BG-174 | Redesenho integral do frontend Layer7 com paridade funcional | Alta | package/GUI + docs + testes | manutenção F4/F5/F7; ondas GUI0–GUI7 | reclamação generalizada; páginas longas, edição fora da dobra, perda de contexto e ambiguidade de efeitos | G | Alto | **GUI0 implementado documentalmente; pendente de gates/commit e GO humano para GUI1** (`2026-08-31`) | SSOT: [`plano-redesign-frontend.md`](plano-redesign-frontend.md); análise, inventário/paridade e wireframes canónicos. Sem código/`PORTVERSION`/build/release; Identity+MITM preservada e fechada. |
