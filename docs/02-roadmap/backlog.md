...
| BG-106 | ABI do builder divergente do appliance | Alta | build/release | F7 / gate de release | pacote FreeBSD 15 exigir `pkg add -f` no pfSense FreeBSD 16, mascarando incompatibilidade binária futura | M | Alto | **Aberto — bloqueia promoção pública de `1.9.31`…`1.9.33`** | build oficial em FreeBSD 15; appliance Plus 26.03.1 / FreeBSD 16. `pkg check`, `ldd`, daemon, config e PF PASS após instalação forçada, mas falta builder/artefato nativo 16 ou verificação formal de compatibilidade. |
| BG-107 | Padronização visual Perfis rápidos + UX única RA + layout admin-block | Media | package/GUI | Caminho A / UX | grupo RA vermelho/ícones vazios/grelha; agregados com nome=grupo; página RA duplicada; Identity/Exceptions/Categories/Reports/Test fora do padrão visual | P | Baixo | **Concluido codigo (`1.9.32`); gate appliance pendente** | cores/ícones/grelha; «(todas/todos)»; redirect RA→Politicas `#l7-ra`; admin-block nas páginas listadas; IDs de política intactos |
| BG-108 | UX visual wave 2 (KPI unificado, subnav Políticas, catálogo, Chart.js offline) | Media | package/GUI | Caminho A / UX | Estado/Relatórios com KPI distintos; Kick/Rumble mal classificados; subpáginas Políticas pouco descobertas; Chart.js via CDN | P | Baixo | **Concluido codigo (`1.9.33`)** — gate appliance/build pendente; promoção bloqueada por BG-106 | KPI partilhado `.l7-kpi-*`; Status/Remoção admin-block; subnav Políticas; kick/rumble→Streaming; meta cat-only; Chart.js local; typo Distracoes; zero enforcement |
