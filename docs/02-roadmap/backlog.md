...
| BG-156 | Completar tradução inglesa do pacote e da página pública de bloqueio | Média | package/GUI | manutenção / F4.3 | cliente escolhe inglês, mas textos de áreas recentes e página de bloqueio ainda regressam ao português | P | Médio | **Implementado; pacote validado, publicação pendente** (`1.9.65`) | Catálogo EN completo para chamadas literais GUI e descrições de perfis; bloqueio público entende `layer7.language`; teste de regressão `test_i18n_coverage.js`; pacote assinado `SHA256=0c12cf38f44347ba69af876eea1e0b3cefc22cde07d9e5fd85ae208876ce0d6f`. Sem daemon, licença ou deploy `.244`. |
