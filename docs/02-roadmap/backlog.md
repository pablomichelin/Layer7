...
| BG-087 | MITM TLS opt-in + CA (IM2) — 20.8–20.11 + D0/D1 + Gate C | Alta | package/GUI | Identity+MITM | GO activação MITM produção (runbook) | M | Alto | **`1.9.46` Gate C PASS** / MITM prod DEFER | anti-QUIC escopo + filter sync; evidência `210753Z`; `.234/.235` proibidos |
