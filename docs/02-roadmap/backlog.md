...
| BG-059 | Aguardar estado final do nDPI antes da decisão | Critica | daemon/captura/testes | Caminho B pré-produção | resultado parcial TLS/QUIC ser congelado antes do refinamento para app/SNI, causando falso allow intermitente | P | Alto | Código e regressões locais concluídos (`1.8.11_31`); builder/appliance pendentes | FP-020; `NDPI_STATE_CLASSIFIED` como contrato; `ndpi_detection_giveup()` no orçamento de 48 pacotes |
