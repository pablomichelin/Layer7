...
| BG-157 | Disponibilizar espanhol como idioma adicional | Média | package/GUI | manutenção / F4.3 | operador hispanofalante não consegue selecionar idioma próprio | P | Baixo | **Implementado; gates e publicação pendentes** (`1.9.66`) | Opção `es`, catálogo espanhol com interface operacional traduzida e fallback EN seguro para mensagens técnicas; página pública de bloqueio em espanhol; daemon, licença e `.244` intactos. |
