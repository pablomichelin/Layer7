...
| BG-009 | Consolidar confiabilidade de package/daemon em reboot, reload, upgrade, rollback e reinicio de servico | Alta | package/daemon | F4 | runtime continuar a divergir entre estado desejado e estado real | G | Alto | Em curso (F4.1) | aberto em F4.0 (2026-04-24); `layer7-stats-collect.sh` (pidfile) e `rc.d/layer7d` (`layer7d_pid_from_file`); `PORTREVISION` 5; roteiro de evidencia **10a** em `validacao-lab.md`; teste **3.8** em `test-matrix.md`; ver `f4-plano-de-implementacao.md` |
| BG-010 | Hardening da trilha de blacklists UT1: download, cron, reload, fallback, except tables e forcing DNS | Alta | blacklists | F4 | subsistema seguir operacionalmente fragil apesar de funcional | G | Alto | Em curso (F4.2) | F4.0 abriu a fase; `send_sighup` com trim de PID + `PORTREVISION` 3; `PLANO-BLACKLISTS-UT1` + `DIRETRIZES` + plano F4; sem bloquear F3/DR-05 |
| BG-011 | Validar forcing DNS e anti-bypass em cenarios reais de VLAN/interface, excepcoes e tabelas PF | Alta | daemon/enforcement | F4 | bypass continuar a aparecer em combinacoes menos comuns | M | Alto | Em curso (F4.3) | blocos 2026-04-24: dedupe/CIDR/guards; `layer7_pf_ifname_for_rules` + log falha `pfctl`; doc MANUAL; matriz de lab ainda aberta |
