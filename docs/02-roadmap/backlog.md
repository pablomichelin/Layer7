...
| BG-028 | Activar pela primeira vez a trust chain F1.2/F1.4 nas releases publicas (manifesto Ed25519 assinado, `install.sh` carimbado fail-closed) | Alta | release-engineering/seguranca | F7 | continuar a publicar pacotes sem manifesto/assinatura como em `v1.7.8` a `v1.8.11_12` | M | Alto | Acompanhar (gate humano) | inclui: gerar par Ed25519 fora do builder e fora do repo (custodia humana), executar `deployz.sh`/`sign-release.sh`/`verify-release.sh`/`publish-release.sh` na proxima release, ADR formal documentando a transicao "publicacao sem manifesto -> publicacao com manifesto", actualizar `MANUAL-INSTALL.md` para reactivar o `install.sh` automatico assinado e o `uninstall.sh`; ate la o caminho oficial e o **comando unico manual** documentado em `MANUAL-INSTALL.md` §1/§4 |
