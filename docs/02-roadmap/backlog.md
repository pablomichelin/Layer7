...
| BG-105 | Reclassificação pós-sinkhole e log storm do portal local | Alta | daemon/logging | F4/hardening | evento enganoso e crescimento desnecessário do log quando o cliente alcança o IP local devolvido pelo DNS | P | Alto | **Código/teste/build concluídos (`1.9.31`); gate appliance pendente** | auditoria única `outcome=sinkhole` no bloqueio DNS; destino local não entra em `layer7_flow_decide`; regressão `test_sinkhole_local_guard.sh`; SHA256 `dcbad868…ffcd15` |
