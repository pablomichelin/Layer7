...
| BG-040 | Caminho A / A1 — inventario de dispositivos read-only (IP<->MAC<->hostname<->vendor via DHCP leases + ARP) + pagina GUI Dispositivos | Alta | package/gui | Caminho A (V2/17) | sem identidade de dispositivo nao ha UX/policy tipo UDM; base para A2 | M | Medio | Em curso (`PORTREVISION=20`; ADR-0011; pagina Dispositivos + `layer7_device_inventory()` DHCP+ARP+OUI+alias; pendente build/appliance) | so leitura; nao altera enforcement |
