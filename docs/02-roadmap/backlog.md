...
| BG-042 | Caminho A / A3 — SNI-aware/CDN: parser TLS ClientHello no daemon + modo politica strict/permissive | Alta | daemon | Caminho A (V2/18) | "host" so vem de DNS; fragil sob DoH/cache/QUIC; falsos positivos em IP CDN partilhado | G | Alto | Em curso (`PORTREVISION=22`; ADR-0013; usa SNI/Host do nDPI — sem parser proprio nem MITM; toggle `sni_inspection` opt-in OFF; pendente build/appliance) | maior risco tecnico; depois da base A0-A2; ECH continua limite |
