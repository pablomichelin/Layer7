...
| BG-150 | P3-9 opção A: documentar «404 esperado» pós-cut `30.11` | Baixa | docs/distribuição | manutenção / pós-auditoria (BG-128) | 404 do espelho confundido com incidente; risco de reupload GA4.11 sem GO | P | Baixo | **Concluido** (`2026-08-14`; sem deploy / `PORTVERSION`) | Nota [`../09-blocking/nota-404-esperado-cut-30.11.md`](../09-blocking/nota-404-esperado-cut-30.11.md). MANUAL-INSTALL / PLANO / DIRETRIZES alinhados. Runtime (`update-blacklists.sh` / `layer7.inc` / `config.json.sample`) intacto. |
