...
| BG-066 | Exclusao de origem por politica (`src_exclude_*`) no daemon/PF | Alta | daemon/package/GUI/PF | Caminho B F4.3 | caso fino "isento so deste perfil" impossivel sem duplicar excepcoes globais | G | Alto | Planeado (Bloco D; candidato `_50`) | ADR-0019; `layer7_pexc_N` scoped; daemon `rule_matches_src`; flush/self-heal completo (licao BG-061) |
