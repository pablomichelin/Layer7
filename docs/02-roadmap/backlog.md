...
| BG-009 | Consolidar confiabilidade de package/daemon em reboot, reload, upgrade, rollback e reinicio de servico | Alta | package/daemon | F4 | runtime continuar a divergir entre estado desejado e estado real | G | Alto | Em curso (F4.1) | `layer7-stats-collect.sh`; `rc.d/layer7d`; PHP `layer7_daemon_pid_from_file` + paginas Status/Diagnostics; `PORTREVISION` 6; roteiro **10a**; teste **3.8**; ver `f4-plano-de-implementacao.md` |
