...
| BG-168 | PF arma enforce sem licença (pedido JSON) | Critica | package/PF | manutenção / F4.3 | modo aplicar + sem `.lic` injecta anti-QUIC/blocks no pfSense; cai a Internet | P | Alto | **Implementado, pendente de gates/commit** (`PORTVERSION=1.9.75`) | `layer7_pf_should_enforce()` = `display_mode===enforce` (daemon armado). Teste `test_pf_should_enforce.php`. Sem overlay `.244`. |
