...
| BG-167 | GUI mostra «aplicar» sem licença (falso positivo) | Alta | package/GUI | manutenção / F4.3 | cliente testa bloqueio, vê vermelho «aplicar» e conclui que o produto falha | P | Alto | **FEITO no git** (`PORTVERSION=1.9.74`, pendente build/release) | Badge/diagnósticos usam `enforce_mode`; sem `enforce_mode=1` nunca «aplicar». |
