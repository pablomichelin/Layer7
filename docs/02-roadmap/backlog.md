...
| BG-072 | Limites daemon excepcoes (`L7_EXC_MAX_HOSTS` 32, `L7_EXC_MAX_CIDRS` 16) | Alta | daemon/package | Caminho B / F4.3 | GUI/upsert aceitam >8 entradas mas daemon trunca silenciosamente (PF ve tudo, decisao DNS so 8+8) | M | Medio | Planeado (Bloco C; candidato `_58`) | unica mudanca C da feature Lista VIP; alinhar validacao PHP e upsert; testes parse/decisao; producao enforce continua `_24` |
