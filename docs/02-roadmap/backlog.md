...
| BG-155 | Correções da revisão de código: lock de check-in, zone-id PF, lockfiles Docker e gate TLS local | Alta | license server + daemon + build | manutenção / F7 | revoke concorrente pode emitir resposta `active`; IP IPv6 com `%iface` chega ao `pfctl`; imagens não reproduzíveis; gate TLS falha fora do OpenSSL padrão | M | Alto | **FEITO no git** (`3762c3c`, `2026-08-15`; build/release pendentes) | Check-in serializa `FOR UPDATE` com operações administrativas; zone-id é rejeitado; Docker usa lockfiles + `npm ci`; runner TLS injeta flags Homebrew. P0-1 continua: não fazer deploy `.244` sem GO. Candidato `1.9.64` requer build FreeBSD + release. |
