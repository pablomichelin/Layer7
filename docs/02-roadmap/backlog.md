...
| BG-174 | Redesenho integral do frontend Layer7 com paridade funcional | Alta | package/GUI + docs + testes | manutenção F4/F5/F7; ondas GUI0–GUI7 | reclamação generalizada; páginas longas, edição fora da dobra, perda de contexto e ambiguidade de efeitos | G | Alto | **GUI0 FEITO `3563757`; ADR-0037 FEITA `f44a14b`; emenda visual nativa implementada, pendente gates/commit; GUI1–GUI7 aguardam GO** (`2026-08-31`) | Formulário pfSense plano: barras escuras, linhas, label esquerda, campo/help direita; sem cards/sticky/design paralelo. Sem código/`PORTVERSION`/build/release; Identity+MITM preservada e fechada. |
