...
| BG-041 | Caminho A / A2 — politicas e grupos por dispositivo (MAC -> IP actual via leases; grupos dinamicos do inventario A1) | Alta | daemon/package/gui | Caminho A (V2/17) | politicas hoje so por IP/CIDR; alvo por dispositivo e nucleo da UX UDM | M | Medio | Em curso (`PORTREVISION=21`; ADR-0012; grupos com `device_macs`->`device_ips`, daemon le `device_ips`, GUI grupos+dispositivos+resync; pendente build/appliance) | reutiliza `groups[]`; resolve MAC->IP no resync/DHCP change |
