...
| BG-159 | Completar inglês de Configurações e defaults da página de bloqueio | Alta | package/GUI | manutenção / F4.3 | textos EN parcialmente em português e defaults PT persistidos após troca de idioma | P | Médio | **Implementado — pendente de gates/release** (`1.9.68`, `2026-08-15`) | Catálogo EN corrigido, defaults internos localizados por idioma e migração limitada aos defaults quando o idioma é alterado. Conteúdo personalizado, daemon, licença e `.244` intactos. |
