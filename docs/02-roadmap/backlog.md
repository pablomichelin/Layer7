...
| BG-011 | Validar forcing DNS e anti-bypass em cenarios reais de VLAN/interface, excepcoes e tabelas PF | Alta | daemon/enforcement | F4 | bypass continuar a aparecer em combinacoes menos comuns | M | Alto | Em curso (F4.3) | blocos 2026-04-24: dedupe/CIDR/guards; `layer7_pf_ifname_for_rules` + log falha `pfctl`; doc MANUAL; `1.8.11_8`–`10`: dedupe (interface,CIDR), ordem estavel de interfaces e CIDRs/regra em `force_dns`; `validacao-lab` sec. **11** com cenário opcional multi-interface / VLAN; matriz de lab ainda aberta |
