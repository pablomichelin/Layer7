...
| BG-036 | Allowlist de destinos (bancos/gov/push) honrada no daemon e no PF (`pass quick to <layer7_allow_dst>`) | Critica | daemon/package/usabilidade | F4 | falsos positivos em IPs partilhados de CDN (bancos, push mobile, gov.br) bloqueados por engano | M | Alto | Concluido (release `v1.8.11_18`; validado no appliance) | novo `src/layer7d/allowlist.{c,h}`; seed `/usr/local/etc/layer7/allowlist-seed.txt`; pagina `layer7_allowlist.php`; tabela PF `layer7_allow_dst`; testes `tests/functional/test_allowlist.c` (24 casos PASS) |
