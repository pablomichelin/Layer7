...
| BG-028 | Activar pela primeira vez a trust chain F1.2/F1.4 do pacote nas releases publicas (manifesto Ed25519 assinado, `install.sh` carimbado fail-closed) | Alta | release-engineering/seguranca | F7 | continuar a publicar pacotes sem manifesto/assinatura como em `v1.7.8` a `v1.8.11_13` | M | Alto | Acompanhar (gate humano) | inclui: gerar par Ed25519 fora do builder e fora do repo (custodia humana), executar `deployz.sh`/`sign-release.sh`/`verify-release.sh`/`publish-release.sh` na proxima release, ADR formal documentando a transicao "publicacao sem manifesto -> publicacao com manifesto", actualizar `MANUAL-INSTALL.md` para reactivar o `install.sh` automatico assinado e o `uninstall.sh`; ate la o caminho oficial e o **comando unico manual** documentado em `MANUAL-INSTALL.md` §1/§4. Nota: este BG cobre apenas o trust chain do **pacote**; o trust chain F1.3 das **blacklists** ja esta activado desde `1.8.11_13` |
...
| BG-034 | Monitor verdadeiramente passivo: gating de `mode`/`enabled` em `layer7_generate_rules()` e `layer7_pf_default_rules_text()` | Critica | package/operacao | F4 | "bloqueia bancos em monitor" — anti-DoT/DoQ e block_dst sempre injectados independentemente do modo | P | Alto | Concluido (branch `1.8.11_18`; aguarda release) | `package/.../layer7.inc`: novo `layer7_pf_should_enforce()`, gates em `layer7_generate_rules()` e `layer7_resync()`; `CHANGELOG.md > [Unreleased]` (Bloco 1) |
| BG-035 | Anti-DoT/DoQ (porta 853) como toggle explicito `block_dot_doq`, OFF por defeito | Alta | package/seguranca | F4 | bloqueio cego de "DNS privado" Android e apps moveis que dependem de DoT (incluindo bancos) | P | Alto | Concluido (branch `1.8.11_18`; aguarda release) | `layer7_bare_config()`, `layer7_pf_default_rules_text()`, `layer7_settings.php`; `CHANGELOG.md > [Unreleased]` (Bloco 2) |
| BG-036 | Allowlist de destinos (bancos/gov/push) honrada no daemon e no PF (`pass quick to <layer7_allow_dst>`) | Critica | daemon/package/usabilidade | F4 | falsos positivos em IPs partilhados de CDN (bancos, push mobile, gov.br) bloqueados por engano | M | Alto | Concluido (branch `1.8.11_18`; aguarda release) | novo `src/layer7d/allowlist.{c,h}`; seed `/usr/local/etc/layer7/allowlist-seed.txt`; pagina `layer7_allowlist.php`; tabela PF `layer7_allow_dst`; testes `tests/functional/test_allowlist.c` (24 casos PASS) |
| BG-037 | Flush fiavel de `layer7_block_dst`, `layer7_block` e `layer7_bld_*` em mudancas de modo, paragem do daemon e SIGKILL | Alta | daemon/package | F4 | IPs `stale` de enforce anterior continuarem a bloquear apos voltar a monitor | P | Alto | Concluido (branch `1.8.11_18`; aguarda release) | `dst_cache_flush()` reforcado + nova `enforcement_flush_all_tables()` em `main.c`; `rc.d/layer7d stop` chama `layer7-pfctl flush-all`; `layer7_resync()` flush quando passivo |
| BG-038 | F5 minima: testes unitarios da allowlist + smoke "monitor nao bloqueia" no appliance | Alta | testes/operacao | F5 | sem rede de seguranca contra regressao do que foi corrigido nos Blocos 1-5 | P | Alto | Em curso (apos build/release `1.8.11_18`) | `tests/functional/test_allowlist.c` (PASS local), `tests/lab/smoke-monitor-mode.sh`, `tests/run-local.sh`; falta extender com casos de policy decision (BG-012/013) |
