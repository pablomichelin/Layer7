...
| BG-058 | Preservar estado nDPI sob buracos/colisões da tabela de fluxos | Critica | daemon/captura/testes | Caminho B pré-produção | mesmo fluxo ganhar dois estados nDPI ou ser descartado silenciosamente, causando classificação intermitente | M | Alto | Código e regressões locais concluídos (`1.8.11_30`); builder/appliance pendentes | FP-019; probe completo antes de inserir; evicção do menos recente; métricas `cap_active/evicted/dropped` no JSON |
