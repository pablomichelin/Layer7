...
| BG-071 | GUI Lista VIP global (descricoes, export/import, atalho SSOT `vip-isentos`) | Alta | package/GUI | Caminho B / UX F4.3 | admin nao regista directores com nome; excepcao escondida no modal; sem export da lista | M | Medio | Concluido codigo (`1.8.11_57` publicado); gate appliance pendente | secao de primeira classe em `layer7_exceptions.php`; labels em `layer7["vip_meta"]["labels"]` (D2 — daemon nunca le); validacao limites coerente com daemon; export/import padrao BG-070; link desde modal Perfis rapidos; aviso DHCP static mapping; producao enforce continua `_24` |
| BG-072 | Limites daemon excepcoes (`L7_EXC_MAX_HOSTS` 32, `L7_EXC_MAX_CIDRS` 16) | Alta | daemon/package | Caminho B / F4.3 | GUI/upsert aceitam >8 entradas mas daemon trunca silenciosamente (PF ve tudo, decisao DNS so 8+8) | M | Medio | Concluido codigo (`1.8.11_58` publicado); gate appliance pendente | unica mudanca C da feature Lista VIP; alinhar validacao PHP e upsert; testes parse/decisao; memoria ~+19 KiB estatica; producao enforce continua `_24` |
| BG-073 | Isencao VIP no caminho DNS (sinkhole Unbound + DNS forcado) | Alta | package/Unbound/PF | Caminho B / F4.3 | VIP na `vip-isentos` continua sujeito a sinkhole global e rdr :53 `from any` | G | Medio | Planeado (Bloco D; candidato `_59`) | ADR-0020; opcao (a) view Unbound preferida; fallback (b) rdr `from !<layer7_exc_allow_N>` com `pfctl -nf`; limitacao sinkhole documentada se (b); gate lab Bloco E; producao enforce continua `_24` |
