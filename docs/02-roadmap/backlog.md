...
| BG-077 | Check-in online periodico e revogacao remota de licenca (cancelamento comercial) | **Critica** | license-server/daemon/licenciamento | **F3+** (pos-campanha; bloqueante comercial recomendado antes GO enforce) | revogacao no servidor nao corta appliance; cliente cancelado continua em enforce ate expiry+grace offline | G | **Alto** | **Planeado** (`2026-08-04`) | ADR-0021 Proposto; plano `f3-plano-check-in-online-revogacao-remota.md`; defaults: check-in 7d, max offline 14d, revogacao efectiva no proximo check-in; endpoint `POST /api/license/check-in`; feature flag rollback; cenario S14; depende de build pacote + deploy `244` |
