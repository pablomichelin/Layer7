...
| BG-170 | Check-in obrigatório (remover opt-out do cliente) | Critica | daemon + package/GUI | manutenção / licença | cliente desliga o toggle e usa o `.lic` até expiry após cancelamento | P | Alto | **Implementado, pendente de gates/commit** (`1.9.77`) | GO produto: filtro de Internet não tem air-gap. Daemon ignora JSON false; GUI sem checkbox; `save_json` força true. N3 intacto. Emenda ADR-0032. Fecha A-04 / BG-101 no cliente. |
