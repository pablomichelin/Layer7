...
| BG-066 | Exclusao de origem por politica (`src_exclude_*`) no daemon/PF | Alta | daemon/package/GUI/PF | Caminho B F4.3 | caso fino "isento so deste perfil" impossivel sem duplicar excepcoes globais | G | Alto | Concluido codigo (`1.8.11_50`; ADR-0019) | `layer7_pexc_N` scoped; daemon `src_matches_rule`; flush/self-heal completo; build/release pendente |
