...
| BG-009 | Consolidar confiabilidade de package/daemon em reboot, reload, upgrade, rollback e reinicio de servico | Alta | package/daemon | F4 | runtime continuar a divergir entre estado desejado e estado real | G | Alto | Em curso (F4.1) | `layer7-stats-collect.sh`; `rc.d/layer7d`; PHP `layer7_daemon_pid_from_file` + paginas Status/Diagnostics; port no branch `1.8.11_12`; roteiro **10a**; teste **3.8**; ver `f4-plano-de-implementacao.md` |
...
| BG-030 | Endurecer o updater do GUI (`layer7_settings.php > check_update`) para ignorar releases que nao sao versoes do pacote | Media | gui/release-engineering | F7 | botao "Verificar actualizacao" mostrar erradamente *"Release encontrado mas sem artefacto .pkg."* sempre que existir uma release nao-pacote (ex.: rolling `blacklists-ut1-current`) marcada como `Latest` | P | Medio | Planeado | hoje o codigo le sempre `releases/latest`; defesa em profundidade: filtrar `tag_name` por `/^v\d+\.\d+/`, ou cair para a primeira release que tenha asset `.pkg`. **Mitigado operacionalmente em `2026-04-24`** via convencao "releases nao-pacote sao publicadas como `prerelease`" (a API `/releases/latest` ignora prereleases); convencao registada em `docs/changelog/CHANGELOG.md` (Operational) e em `docs/10-license-server/MANUAL-INSTALL.md` §11b.1. Este BG fica para a proxima versao do pacote materializar a defesa no codigo |
