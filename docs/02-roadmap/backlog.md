...
| BG-155 | Correções da revisão de código: lock de check-in, zone-id PF, lockfiles Docker e gates locais/release | Alta | license server + daemon + build | manutenção / F7 | revoke concorrente pode emitir resposta `active`; IP IPv6 com `%iface` chega ao `pfctl`; imagens não reproduzíveis; gates podem falhar por fontes/aceite de licença divergentes | M | Alto | **FEITO no git** (`3762c3c`, `8b70bb9`, próximo commit; `2026-08-15`; release pendente) | Check-in serializa `FOR UPDATE` com operações administrativas; zone-id é rejeitado; Docker usa lockfiles + `npm ci`; runner TLS injeta flags Homebrew; smoke inclui o gate de licença e `deployz` não depende de confirmação interativa. P0-1 continua: não fazer deploy `.244` sem GO. Candidato `1.9.64` requer stage assinado + release. |
