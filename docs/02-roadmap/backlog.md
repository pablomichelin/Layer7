...
| BG-163 | Cliente install-ping `1.9.71` falha em silêncio | Alta | package/daemon | manutenção / F7 | endpoint live 200 mas Instalações vazia; `config.inc` CLI + curl fora do PATH + retry 24 h | M | Alto | **Implementado, pendente de gates/commit** (`1.9.72`) | Lê `config.xml`; `php -f`; `/usr/local/bin/curl`; fallback `hardware_id`; tick 15 min + throttle PHP. Sem publish / sem overlay `.244`. |
