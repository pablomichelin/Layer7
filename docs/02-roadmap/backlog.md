...
| BG-174 | Redesenho integral do frontend Layer7 com paridade funcional | Alta | package/GUI + docs + testes | manutenção F4/F5/F7; ondas GUI0–GUI7 | reclamação generalizada; páginas longas, edição fora da dobra, perda de contexto e ambiguidade de efeitos | G | Alto | **GUI0 FEITO no git `3563757`; addendum ADR-0037 implementado, pendente de gates/commit; GUI1–GUI7 aguardam GO** (`2026-08-31`) | Frontend obrigatoriamente nativo ao pfSense; manter organização/tabs/tema/componentes. Cinco áreas = taxonomia, não shell. Sem código/`PORTVERSION`/build/release; Identity+MITM preservada e fechada. |
