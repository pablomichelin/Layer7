...
**Estado:** plano fecho **FECHADO** (`2026-08-05`); produção enforce **`1.9.8`** (GV7.4).
...
**Trilha Identity + MITM Add-on:** [`plano-identity-mitm-addon.md`](plano-identity-mitm-addon.md) — **【FILA FECHADA】** (`20.37`).  
...
**A1/A2/M2 FEITO** no git (`28c97ad` + governação após gates; `2026-08-14`; lifecycle
...
`X-Forwarded-Proto $scheme`; sem deploy / `PORTVERSION`). **P1-9 AVALIADO**
no git (`2026-08-14`; residual pós-P2-3 não aberto no HEAD; sem mudança de
...
`PORTVERSION`). **P3-3A FEITO** no git
(`2026-08-14`; `/login` disabled e inexistente partilham 401 genérico +
bcrypt + `registerLoginFailure`; sem deploy / `PORTVERSION`).
**P3-3B** FEITO no git
(`2026-08-14`; `POST`/`PUT /api/users` exige password >=12; `/login` não
...
`Erro interno.`; sem deploy / `PORTVERSION`).
**P3-5** FEITO no git
(`2026-08-14`; promoção atómica do `.lic` em Activate — tmp 0600 +
...
runtime).
**P3-9 AVALIADO** no git
(`2026-08-14`; **BG-150**; opção A — **FEITO documental**; URLs
**não** removidos; sem mudança de runtime).
**P2-16 AVALIADO** no git
(`2026-08-14`; **BG-151**; opção A — **FEITO documental**; rollback
preferido = overlay `bbc74a5…`; tag `pre-30.13` **não** é
padrão/`latest`; sem tag/retag/deploy).
**P2-14 AVALIADO** no git
(`2026-08-14`; **BG-152**; opção A — **FEITO documental**; bypass ABI
`-f` = política BG-106; builder FreeBSD 16 **não** provado; sem
código/`PORTVERSION`).
**P3-7 AVALIADO** no git
...
GUI1 piloto Status **PUBLICADO `v1.9.80`** (`releases/latest`; SHA256
...
Emenda frontend-only de `2026-08-31` **FEITA no git** (`3b18f82`) após gates
PASS: o produto e os efeitos de servidor ficam congelados. Cada onda reorganiza
somente a view, consome `head.inc`/`foot.inc`, `Form_*` e assets já fornecidos
pelo pfSense, com alvo de zero CSS visual próprio e medição de recursos por rota.
...
- **Lista VIP global (`2026-07-31`, documental Bloco A):** extensão da isenção
  ...
  **BG-071** (GUI Lista VIP, `_57`), **BG-072** (limites `L7_EXC_MAX_*`,
  ...
  inalterado; labels em `vip_meta.labels`. Ordem A→E; Blocos A–E concluídos
  (codigo `_57`–`_59` + roteiro lab sec. **20**); execução física no appliance
  pendente validação humana. NO-GO producao inalterado.
