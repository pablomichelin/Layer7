...
| Campo | Regra |
|-------|--------|
| `mode` | `monitor` \| `enforce` (pedido gravado). O badge **e o PF** só armam com `enforce_mode=1` do daemon (BG-167 / BG-168). Sem licença = monitorizar, sem anti-QUIC nem `block drop`. |
