...
**P3-3A FEITO** no git (`2026-08-14`) — `POST /api/auth/login` deixa de
...
`verifyTotp` compara HOTP/TOTP com Buffer UTF-8 + guarda de
comprimento + `timingSafeEqual`. Residual P3-4 (fechado neste bloco). Sem deploy /
`PORTVERSION`.
...
**Veredicto:** P3-9 **AVALIADO** (**BG-150**; opção A — **FEITO
documental**). A tag foi cortada; 404 anónimo é esperado; primary
exige token; isto **não** é o canal do pacote nem motivo para
reupload GA4.11. O espelho no cliente é **legado / fallback de
runtime**. URLs **não** removidos.
Nota [`nota-404-esperado-cut-30.11.md`](nota-404-esperado-cut-30.11.md).
Evidência [`../tests/evidence/20260814T204500Z-p39-404-esperado/`](../tests/evidence/20260814T204500Z-p39-404-esperado/).
...
| Teste | Backend 258/258 PASS; frontend 59/59 PASS + build; regressão C de zone-id PASS; `git diff --check` PASS; builder FreeBSD: pubkey, port-files, smoke e `make package` PASS. Stage oficial `1.9.64` criado, SHA256 `692ab615b0a45f70958f2b866d339e44f833f7953aeec5f780ee0af9e5afeb5f`. Código **FEITO no git** (`3762c3c`, `8b70bb9`, `f0cf615`). Assinatura e release pendentes. |
