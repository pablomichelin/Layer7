...
if ($_POST["add_group"] ?? false) {
	...
	if (count($groups) >= 16) {
		$input_errors[] = gettext("Limite de 16 grupos.");
		$ok = false;
	}
	...
	if ($ok) {
		foreach ($groups as $existing) {
			if (isset($existing["id"]) && (string)$existing["id"] === $gid) {
				$input_errors[] = gettext("Ja existe um grupo com esse ID.");
				$ok = false;
				break;
			}
		}
	...
	if ($ok && empty($cidrs) && empty($hosts)) {
		$input_errors[] = gettext("Indique pelo menos um CIDR ou IP.");
		$ok = false;
	}
	...
	if ($ok) {
		...
		if (layer7_save_json($data)) {
			...
			$savemsg = gettext("Grupo adicionado.");
		}
	}
	unset($groups);
}
...
if ($_POST["delete_group"] ?? false) {
	...
	if ($idx < 0 || $idx >= count($groups)) {
		$input_errors[] = gettext("Indice de grupo invalido.");
	} else {
		...
		if ($in_use) {
			$input_errors[] = sprintf(gettext("O grupo '%s' esta em uso por uma politica. Remova a referencia antes de apagar."), $del_id);
		} else {
			array_splice($groups, $idx, 1);
			if (layer7_save_json($data)) {
				layer7_signal_reload();
				$savemsg = gettext("Grupo removido.");
			}
		}
	}
	unset($groups);
...
$pgtitle = array(gettext("Services"), gettext("Layer 7"), gettext("Grupos"));
include("head.inc");
layer7_render_styles();
?>
<div class="panel panel-default layer7-page">
	<div class="panel-heading">
		<h2 class="panel-title"><?= gettext("Layer 7 - Grupos de dispositivos"); ?></h2>
	</div>
	<div class="panel-body">
		<?php layer7_render_tabs("groups"); ?>
		<div class="layer7-content">
			<?php layer7_render_messages(); ?>

			<p class="layer7-lead"><?= gettext("Crie grupos nomeados de dispositivos (ex.: Funcionarios, Visitantes) e aplique politicas por grupo em vez de repetir CIDRs manualmente."); ?></p>

		<div class="layer7-section">
			<h3 class="layer7-section-title"><?= gettext("Grupos actuais"); ?></h3>
			<?php if (count($groups) === 0) { ?>
			<div class="alert alert-info"><?= gettext("Nenhum grupo criado. Adicione o primeiro grupo abaixo."); ?></div>
			<?php } else { ?>
					...
					?>
						<tr>
							...
							<td class="layer7-table-actions">
								<a href="layer7_groups.php?edit=<?= (int)$i; ?>" class="btn btn-xs btn-info"><?= gettext("Editar"); ?></a>
							</td>
						</tr>
					<?php } ?>
					</tbody>
			...
			<form method="post" class="form-inline layer7-inline-form"
				onsubmit='return confirm(<?= json_encode(gettext("Remover este grupo?")); ?>);'>
				<div class="form-group">
					<label class="control-label" for="delete_group_index"><?= gettext("Remover grupo"); ?></label>
					...
					<button type="submit" name="delete_group" value="1" class="btn btn-danger"><?= gettext("Remover"); ?></button>
				</div>
			</form>
			<?php } ?>
		</div>
		...
		?>
		<div class="layer7-section">
			<h3 class="layer7-section-title"><?= gettext("Editar grupo"); ?></h3>
			<div class="layer7-toolbar">
				<a href="layer7_groups.php" class="btn btn-default"><?= gettext("Cancelar edicao"); ?></a>
			</div>
			<form method="post" class="form-horizontal">
				...
				<div class="form-group">
					<label class="col-sm-3 control-label"><?= gettext("IPs individuais"); ?></label>
					<div class="col-sm-6">
						...
						<p class="help-block"><?= gettext("Um IPv4 por linha (max. 16)."); ?></p>
					</div>
				</div>

				<div class="form-group">
					<div class="col-sm-offset-3 col-sm-9">
						<button type="submit" name="save_group_edit" value="1" class="btn btn-primary"><?= gettext("Guardar alteracoes"); ?></button>
					</div>
				</div>
			</form>
		</div>
		...
		<div class="layer7-section">
			<h3 class="layer7-section-title"><?= gettext("Adicionar grupo"); ?></h3>
			<p class="layer7-lead"><?= gettext("Defina um grupo de dispositivos com CIDRs e/ou IPs individuais. Depois associe o grupo a politicas na pagina de politicas."); ?></p>
			<?php if ($at_limit) { ?>
			<div class="alert alert-warning"><?= gettext("Limite de 16 grupos atingido."); ?></div>
			<?php } else { ?>
			<form method="post" class="form-horizontal">
				...
				<div class="form-group">
					...
					<div class="col-sm-6">
						...
						<p class="help-block"><?= gettext("Identificador unico (letras, numeros, _ e -)."); ?></p>
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-6">
						<input type="text" name="new_group_name" class="form-control" maxlength="160"
							placeholder="<?= gettext("Ex.: Funcionarios"); ?>" />
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-6">
						...
						<p class="help-block"><?= gettext("Um CIDR por linha (max. 8). Ex.: 10.0.85.0/24."); ?></p>
					</div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-6">
						...
						<p class="help-block"><?= gettext("Um IPv4 por linha (max. 16). Opcional se ja tiver CIDRs."); ?></p>
					</div>
				</div>

				<div class="form-group">
					<div class="col-sm-offset-3 col-sm-9">
						<button type="submit" name="add_group" value="1" class="btn btn-success"><?= gettext("Adicionar grupo"); ?></button>
					</div>
				</div>
			</form>
			<?php } ?>
