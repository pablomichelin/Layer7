	...
	<div class="panel-body">
		...
		<div class="panel panel-default" id="l7-policies">
			...
			<p>
				...
				<a href="layer7_policies.php#l7-profiles" class="btn btn-default"><?= htmlspecialchars(l7_t("Abrir biblioteca de perfis")); ?></a>
			</p>
			...
			<p>
				<a data-toggle="collapse" href="#l7-delete-policy" class="text-danger">
					<i class="fa fa-trash"></i> <?= l7_t("Remover politica"); ?>
				</a>
			</p>
			<div id="l7-delete-policy" class="collapse">
		...
		<div class="panel panel-default" id="l7-profiles">
			...
			<p>
				<input type="text" id="l7ProfileSearch" class="form-control" placeholder="<?= l7_t("Pesquisar perfil..."); ?>" autocomplete="off" oninput="l7filterProfileGrid();" />
			</p>
			<p>
				<label>
		...
		?>
			<tr class="<?= $card_classes; ?>" title="<?= $card_title; ?>"
				...
				<td>
				<?php if (!$prof_exists) { ?>
				<button type="button" class="btn btn-xs btn-default" title="<?= l7_t("Opcoes"); ?>" onclick="l7showProfileModal(<?= htmlspecialchars(json_encode($prof_id_raw), ENT_QUOTES) ?>, <?= htmlspecialchars(json_encode($prof["name"] ?? $prof_id_raw), ENT_QUOTES) ?>);"><?= l7_t("Opcoes"); ?></button>
				<?php } ?>
				<button type="button" class="btn btn-xs btn-default" title="<?= l7_t("Editar perfil"); ?>" onclick="l7showProfileEditModal(<?= htmlspecialchars(json_encode($prof_id_raw), ENT_QUOTES) ?>, false);"><?= l7_t("Editar"); ?></button>
				<?php if ($is_hidden) { ?>
				<form method="post" action="layer7_policies.php#l7-profiles">
					<input type="hidden" name="profile_id" value="<?= $prof_id; ?>" />
		...
		$l7_render_profile_group = function ($gname, $gprofs, $opts = array()) use ($l7_render_profile_card, $l7_group_slug, $l7_count_group_active) {
			...
			$initial_open = false;
			$group_classes = "panel panel-default l7-profile-group l7-profile-group-collapsed";
			if ($is_presets) {
		...
		?>
			<div class="<?= $group_classes; ?>"<?= $anchor_attr; ?> data-group-id="<?= htmlspecialchars($gid); ?>" data-group-default-open="0">
				<div class="panel-heading l7-profile-group-header" role="button" tabindex="0" aria-expanded="false" onclick="l7toggleProfileGroup(this);" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();l7toggleProfileGroup(this);}">
				...
				<div class="panel-body l7-profile-group-body" hidden>
		...
		?>
		<div class="panel panel-default" id="l7-edit">
			<div class="panel-heading">
				<h2 class="panel-title"><?= htmlspecialchars(l7_t("Editar politica")); ?></h2>
			</div>
			<div class="panel-body">
			<p><?= l7_t("Atualize os detalhes da regra selecionada. O identificador permanece fixo para manter a referencia no JSON."); ?></p>
			<p>
				<a href="layer7_policies.php" class="btn btn-default"><?= htmlspecialchars(l7_t("Voltar a lista")); ?></a>
			</p>
		...
		<div class="panel panel-default" id="l7-add">
			<div class="panel-heading">
				<h2 class="panel-title"><?= htmlspecialchars(l7_t("Adicionar politica")); ?></h2>
			</div>
			<div class="panel-body">
			<p>
				<a href="layer7_policies.php" class="btn btn-default"><?= htmlspecialchars(l7_t("Voltar a lista")); ?></a>
			</p>
			<p><?= l7_t("Use nomes claros e prioridades previsiveis para manter a leitura do conjunto simples durante o troubleshooting."); ?></p>
			<?php if ($at_limit) { ?>
			...
			<p class="help-block"><?= l7_t("Para alterar o id de uma politica existente, edite /usr/local/etc/layer7.json diretamente."); ?></p>
			</div>
		</div>
		<?php } ?>
		</div>
